...
///////////////////////////////////////////////////////////////////////////////////////
void	TLSConstraintIdentifier::initCnstrIdentifier(const LSCalcDataSet& lsds, TLGCDataSet& data)
{
	//Initialise elements
	bool T, F;
	T = true;
	...
	LSPosVecConstIter iter = lsds.beginPV();
	LSPosVecConstIter iterEnd = lsds.endPV();
	...
	if(	data.getSDDimension() > 0 ||
		data.getEDMSDDimension() > 0 ||
		data.getHDDimension() > 0 ||
		data.getVDDimension() > 0 ||
		data.getVDDimension() > 0 ||
		data.getLevelObsDimension() > 0 ||
		data.getOffsetToVerLineDimension() > 0 ||
		data.getOffsetToSpaLineDimension() > 0 ||
		data.getOffsetToVerPlaneDimension() > 0 ||
		data.getOffsetToTheoPlaneDimension() > 0 )
	{	
		if(bscale == false)
		{bscale =true;}
	}
	...
	if( data.getGyroOrieDimension() > 0 ||
