...
TReal		TLGCObsSummary::getMeanLoLimit()
{
	...
	if( dof > 0 )
	{
		...
		//STLo = nag_deviates_students_t(Nag_LowerTail,(LITERAL(1.0)-prob), dof, fail);
...
TReal		TLGCObsSummary::getMeanHiLimit()
{
	...
	if( dof > 0 )
	{
		...
		// calculated the confidence limits from the Student T distribution
		//STHi = nag_deviates_students_t(Nag_UpperTail,(LITERAL(1.0)-prob), dof, fail);
...
TReal		TLGCObsSummary::getVarLoLimit()
{
	...
	if( dof > 0 )
	{
		...
		//chiLo = nag_deviates_chi_sq(prob, dof, fail);
...
TReal		TLGCObsSummary::getVarHiLimit()
{
	...
	if( dof > 0 )
	{
		...
		// chi test coefficients
		//chiHi = nag_deviates_chi_sq((1-prob), dof, fail);
