////////////////////////////////////////////////////////////////////
// TOffsetToVerPlaneConverter.cpp
/*!
...
//ClassImp(TOffsetToVerPlaneConverter)
...
/////////////////////////////////////////////////////////////////////////////
//default constructor
TOffsetToVerPlaneConverter::TOffsetToVerPlaneConverter(TAStreamFormatter& stream) :
...
//destructor
TOffsetToVerPlaneConverter::~TOffsetToVerPlaneConverter()
{}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
void	TOffsetToVerPlaneConverter::writeResultsHeader(const int)
{
	...
	(*stream).writeStringLeft(nameWidth,	"POINT 3"); //third point's Name
...
void	TOffsetToVerPlaneConverter::writeResults(LSOffsetToVerPlaneConstIter obsIt)
{
	...
	//write Point 3
	(*stream).writeStringLeft(nameWidth, obsIt->getSecondTgPointName());
...
//RELIABILITY
/////////////////////////////////////////////////////////////////////////
void	TOffsetToVerPlaneConverter::writeReliabilityData(TLSCalcOffsetToVerPlaneObservation& obs)
{
	this->TLengthObsConverter::writeReliabilityData(	obs,
														obs.getFirstTgPointName(),
														obs.getStPointName(),
														obs.getSecondTgPointName());
	return;
}


void	TOffsetToVerPlaneConverter::writeReliabilityHeader()
{
	this->TObservationConverter::writeReliabilityHeader("POINT 1",
														"STATION",
														"POINT 2",
														"OBSERVE",
														"M",
...
//PUNCH MES
/////////////////////////////////////////////////////////////////////////
void	TOffsetToVerPlaneConverter::writeMesData(LSOffsetToVerPlaneConstIter obsIt)
{
	TAngle gis, noV0;
...
//READ FUNCTION
////////////////////////////////////////////////////////////////////////////////////////////////////
int TOffsetToVerPlaneConverter::readObservation(int i, int& numOfMeas, TLGCProject* project)
{
	TAStreamFormatter*	stream = getStream();
...
//	TOffsetToVerPlaneMeasurement* newOffset = new TOffsetToVerPlaneMeasurement;
	...
	if(noColSig)
	{
		project->getDataSet()->setDefaultOffsetToVerPlaneSig(true);
	}
	...
	while ((stream->peek()) != '*')
	{
		...
		else
		{
			...
			TSpatialPointName tgSecondPointName;
			...
			readPtName(tgSecondPointName);

			readMeasurement(obsOffset);
			readSigma(sigma);

			//create the offset measurement
			TSpatialPlaneName tgPlaneName (tgFirstPointName, tgSecondPointName);
			TOffsetToVerPlaneMeasurement* newOffset = new TOffsetToVerPlaneMeasurement(tgPlaneName, obsOffset, sigma);
			...
			insert = insertOffsetToVerPlaneMeas(newDistSt, newOffset, project, i);

			if (insert)
			{
				project->getDataSet()->addToOffsetToVerPlaneNum();
			}
			delete newOffset;
		}
		stream->readLine();
...
bool TOffsetToVerPlaneConverter::insertOffsetToVerPlaneMeas(TDistMeasStation* newDistSt, TOffsetToVerPlaneMeasurement* newOffset,
															TLGCProject* project, int i)
{
	...
	if (project->getDataSet()->getWorkingPoints()->getPoint(newDistSt->getStationedPoint()) != iterPtEnd)
	{
		...
		{
			if (project->getDataSet()->getWorkingPoints()->getPoint(newOffset->getTargetPlaneName().getSecondPointName()) != iterPtEnd)
			{
				// initialize the station with offset measurement
				newDistSt->addOffsetToVerPlaneMeas(newOffset);
			...
			else
			{
				...
				line = "line number " + iLine + " : ";
				string error = project->getError();
				error=error+line+ "Targeted Plane doesn't exist" + '\n';
				project->setError(error);
			}
		}
		else
