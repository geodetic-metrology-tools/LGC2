...
//ClassImp(TOffsetToSpaLineConverter)
...
/////////////////////////////////////////////////////////////////////////////
//default constructor
TOffsetToSpaLineConverter::TOffsetToSpaLineConverter(TAStreamFormatter& stream) :
...
//destructor
TOffsetToSpaLineConverter::~TOffsetToSpaLineConverter()
{}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
void	TOffsetToSpaLineConverter::writeResultsHeader(const int)
{
...
void	TOffsetToSpaLineConverter::writeResults(LSOffsetToSpaLineConstIter obsIt)
{
...
//RELIABILITY
/////////////////////////////////////////////////////////////////////////
void	TOffsetToSpaLineConverter::writeReliabilityData(TLSCalcOffsetToSpaLineObservation& obs)
{
...
void	TOffsetToSpaLineConverter::writeReliabilityHeader()
{
...
//PUNCH MES
/////////////////////////////////////////////////////////////////////////
void	TOffsetToSpaLineConverter::writeMesData(LSOffsetToSpaLineConstIter obsIt)
{
	TAngle gis, noV0;
...
//READ FUNCTION
////////////////////////////////////////////////////////////////////////////////////////////////////////
int TOffsetToSpaLineConverter::readObservation(int i, int& numOfMeas, TLGCProject* project)
{
	TAStreamFormatter*	stream = getStream();
...
//	TOffsetToSpaLineMeasurement* newOffset = new TOffsetToSpaLineMeasurement;
	...
	if(noColSig)
	{
		project->getDataSet()->setDefaultOffsetToSpaLineSig(true);
	}
	...
	while ((stream->peek()) != '*')
	{
		...
		else
		{
			...
			TSpatialLineName tgLineName (tgFirstPointName, tgSecondPointName);
			
			TOffsetToSpaLineMeasurement* newOffset = new TOffsetToSpaLineMeasurement(tgLineName, obsOffset, sigma);
			...
			insert = insertOffsetToSpaLineMeas(newDistSt, newOffset, project, i);

			if (insert)
			{
				project->getDataSet()->addToOffsetToSpaLineNum();
			}
			delete newOffset;
		}
		stream->readLine();
...
bool TOffsetToSpaLineConverter::insertOffsetToSpaLineMeas(TDistMeasStation* newDistSt, TOffsetToSpaLineMeasurement* newOffset,
														  TLGCProject* project, int i)
{
	...
	if (project->getDataSet()->getWorkingPoints()->getPoint(newDistSt->getStationedPoint()) != iterPtEnd)
	{
		...
		{
			if (project->getDataSet()->getWorkingPoints()->getPoint(newOffset->getTargetLineName().getSecondEndName()) != iterPtEnd)
			{
				// initialize the station with offset measurement
				newDistSt->addOffsetToSpaLineMeas(newOffset);
