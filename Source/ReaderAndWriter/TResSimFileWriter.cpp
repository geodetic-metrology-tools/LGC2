...
{//Write each simulation summary -for all but the last simulation, if there no errors in the project
	...
	for( i=0; i < penultimate; i++)
	{
		...
		//Write Points
		this->writePointsResults(project.getCalcParams()->getRefSurfaceName(), *project.getDataSet(), *calcRec);

		// Write instrument summaries
		list<InstrumentsCalcRecord*> iRecord = calcRec->getInstrumentsCalcRecords();
		list<InstrumentsCalcRecord*>::iterator instrIter = iRecord.begin();

		while (instrIter != iRecord.end())
		{
			writeInstrumentsSummary(project, *instrIter);
...
void	TResSimFileWriter::writeLastSimResult(TLGCProject &project)
{
	...
	this->writePointsResults( project.getCalcParams()->getRefSurfaceName(), *project.getDataSet(), *lsds);

	// Write Observations Results
	this->writeObsResults( *lsds, project );
