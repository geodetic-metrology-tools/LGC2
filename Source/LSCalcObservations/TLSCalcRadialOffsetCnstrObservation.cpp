...
{//Default constructor
	...
	fSigmaAPriori = TLength(LITERAL(0.0));
	fSigmaAPosteriori = TLength(LITERAL(0.0));
	fResidue = TLength(LITERAL(0.0));
	fResVariance = TDouble(LITERAL(0.0));
	// TODO: commented the next line
	//fPoint = 0;
}


TLSCalcRadialOffsetCnstrObservation::TLSCalcRadialOffsetCnstrObservation(RadOffCnstrObsConstIter obs,
									LSPosVecIter point): 
fRadialOffsetCnstrObs(obs),fPoint(point)
{
	fSigmaAPriori = obs->getRadialOffsetCnstrSigma();
