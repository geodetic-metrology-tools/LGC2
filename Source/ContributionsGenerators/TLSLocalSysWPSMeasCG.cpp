...
#include  "TLSLocalSysWPSMeasCG.h"
////////////////////////////////////////////////////////////////


//ClassImp(TLSLocalSysWPSMeasCG)
...
//////////////////////////////////////////////////////////////////////
TLSLocalSysWPSMeasCG::TLSLocalSysWPSMeasCG()
{	// default constructor
...
/////////////////////////////////////////
TLSLocalSysWPSMeasCG::TLSLocalSysWPSMeasCG(TLSInputMatrices* lsim):
TALSWPSMeasContribGenerator(lsim){

}

// **This needs to be implemented**
TLSLocalSysWPSMeasCG::TLSLocalSysWPSMeasCG( const  TLSLocalSysWPSMeasCG& original ):
TALSWPSMeasContribGenerator(original)
{	// copy constructor
...
//////////////////////////////////////////////////////////////////////
// **This needs to be implemented**
TLSLocalSysWPSMeasCG&  TLSLocalSysWPSMeasCG::operator=(const TLSLocalSysWPSMeasCG& right)
...
//////////////////////////////////////////////////////////////////////////
// calculates and adds the WPS measurement's contributions to the matrices
//////////////////////////////////////////////////////////////////////////
void TLSLocalSysWPSMeasCG::processWPSMeas(TLSCalcWPSStation::CalcWPSMeasIterator iwm){
	...
	// gets the wire's first end's coordinates, status and indices
	double xE1, yE1, zE1;
	MatrixIndex xE1Ind, yE1Ind, zE1Ind;
	TSpatialPosition::ECoordStatus stsxE1, stsyE1, stszE1;

	TLSCalcWorkingPoints::CalcPtConstIter iE1 = iwm->getWire()->getFirstEnd();
	...
	TLSCalcWorkingPoints::CalcPtConstIter iE2 = iwm->getWire()->getSecondEnd();
	...
	//Transverse Offset
	MatrixIndex tOffEInd = iwm->getTOEqnIndex();
	MatrixIndex tOffOInd = iwm->getTOObsIndex();

	//Vertical Offset
	MatrixIndex vOffEInd = iwm->getVOEqnIndex();
	MatrixIndex vOffOInd = iwm->getVOObsIndex();

	
	//Checks if the contributions of the transverse offset can be calculated
	// if yes, calculates them

	//***
	// Transverse Offset Measurement
	//***
	if (xStk && yStk && xE1k && yE1k && xE2k && yE2k && (tOffOInd!=0)){

		//calculated offset
		double numerator = (xE2 - xE1)*(ySt - yE1) - (yE2 - yE1)*(xSt - xE1);
		double d12h = dist(TLength(xE1), TLength(yE1), TLength(xE2), TLength(yE2));
		double calcTOff = numerator/d12h;


		//**gets the observation's provisinal value
		//checks if the observed value has to be simulated
		double sigmaTOff = (iwm->getTOffsetSigmaAPriori()).getMetresValue();
		double obsTOff;
		if (fSimulate && (iwm->firstIteration())){
			obsTOff = getSimulatedValue(calcTOff, sigmaTOff);
			TLength temp(obsTOff);
			iwm->setSimulatedObsTOffset(temp);
		}
		else
			obsTOff = (iwm->getObsTransOffset()).getMetresValue();
		...
		if (stsxSt == TANumericValue::kVariable){
			...
			fInputMatrices->setFirstDgnMtrxElement(tOffOInd, xStInd, a);
		}

		//ySt coefficient
		if (stsySt == TANumericValue::kVariable){
			double b = (xE2 - xE1)/d12h;
			fInputMatrices->setFirstDgnMtrxElement(tOffOInd, yStInd,b);
		}

		//xE1 coefficient
		if (stsxE1 == TANumericValue::kVariable){
			double c = (yE2 - ySt)/d12h	+ (xE2 - xE1)*numerator/pow(d12h,3);
			fInputMatrices->setFirstDgnMtrxElement(tOffOInd, xE1Ind,c);
		}

		//yE1 coefficient
		if (stsyE1 == TANumericValue::kVariable){
			double d = (xSt - xE2)/d12h	+ (yE2 - yE1)*numerator/pow(d12h,3);
			fInputMatrices->setFirstDgnMtrxElement(tOffOInd, yE1Ind,d);
		}

		//xE2 coefficient
		if (stsxE2 == TANumericValue::kVariable){
			double e = (ySt - yE1)/d12h	- (xE2 - xE1)*numerator/pow(d12h,3);
			fInputMatrices->setFirstDgnMtrxElement(tOffOInd, xE2Ind,e);
		}

		//yE2 coefficient
		if (stsyE2 == TANumericValue::kVariable){
			double f = (xE1 - xSt)/d12h	- (yE2 - yE1)*numerator/pow(d12h,3);
			fInputMatrices->setFirstDgnMtrxElement(tOffOInd, yE2Ind,f);
		}


		//** Adding the contribution to the second design matrix
		fInputMatrices->setSecondDgnMtrxElement(tOffEInd, tOffOInd, -1);


		//** setting the misclosure vector element
		double k = calcTOff - obsTOff;
		fInputMatrices->setMisclosureVectorElement(tOffOInd, k);

		//** adding the contributions to the weight matrix
		fInputMatrices->setWeightMtrxElement(tOffOInd, tOffOInd, pow(fS0APrioriScaleFactor,2)/pow(sigmaTOff,2));
	}
//	else
//		cerr << "WPSMeas : transverse offset contributions can't be calculated : not enough known coordinates\n";
	...
	if (xStk && zStk && xE1k && zE1k && xE2k && zE2k && (vOffOInd!=0)){
		...
		//**gets the observation's provisinal value
		//checks if the observed value has to be simulated
		double sigmaVOff = (iwm->getVOffsetSigmaAPriori()).getMetresValue();
		double obsVOff;
		if (fSimulate && (iwm->firstIteration())){
			obsVOff = getSimulatedValue(calcVOff, sigmaVOff);
			TLength temp(obsVOff);
			iwm->setSimulatedObsVOffset(temp);
		}
		else
			obsVOff = (iwm->getObsVertOffset()).getMetresValue();
...
//	else
//		cerr << "WPSMeas : vertical offset contributions can't be calculated : not enough known coordinates\n";
