...
/*!
	...
	Values given in the input file can be in various units, but they are transformed during reading into metres[m] (lengths) or radians[rads] (angles), in these units they are stored
*/
class TInstrumentData {
	public:
		...
		/// All avaliable polar instruments, accessible by their ID. See \ref getDevice for failsave lookup.
		std::map<const std::string, TPOLAR> fPOLAR;
		/// All avaliable distance meters, accessible by their ID. See \ref getDevice for failsave lookup.
		std::map<const std::string, TEDM>   fEDM;
		/// All avaliable leveling instruments, accessible by their ID. See \ref getDevice for failsave lookup.
		std::map<const std::string, TLEVEL> fLEVEL;
		/// All avaliable scale devices, accessible by their ID. See \ref getDevice for failsave lookup.
