...
int TLGCApp::exec()
{
	...
	{
		fProjectData->fOutputFileWriter.setOutputfileLocation(floggerfileLoc);
		fProjectData->fOutputFileWriter.writeReportHeader("LGC output file");
...
project->getDataSet()->setFileParams(project->getResultsFileParams());
	...
	//This can be reset if SEP keyword in LGC1, this is default one
	...
	if (!dat->fOutputFileWriter.hasErrors())
		file->writeFile();
	else
