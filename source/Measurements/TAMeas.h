...
class TAdjustablePoint;
	...
	/// Enum for the case that no value of a certain type is measured in \ref TAMeas.
	enum ENoValues {};
	/// Enum for the case that only a single value of a certain type is measured in \ref TAMeas .
	enum ESingleValue {
		kValue ///< Symbolic access to a single value in a measurement
	};
/*!
		Base class for measurement values. A single measurement may consist
	...
	*/
	...
	class TAMeas { 
		...
		public:
			...
			*/
			TAMeas(const TAdjustablePoint& targetPos, typename TTarget tgt, TReal value) :
				target(tgt),
				targetPos(&targetPos),
				line(NO_VALi),
				fFirstEquationIndex(NO_VALi),
			...
			void setFirstEquationIndex(MatrixIndex firstEquationIndex){
				fFirstEquationIndex = firstEquationIndex;
			}

			/// Returns LS-matrices equation index of a first equation of this measurement
			MatrixIndex getFirstEquationIndex ()const{
				return fFirstEquationIndex;
			}

			/// Returns LS-matrices observation index of a first observation of this measurement
			MatrixIndex getFirstObservationIndex ()const{
				return fFirstObservationIndex;
			}

			/// Sets LS matrices equation index of a first equation of this measurement
			void setFirstObservationIndex(MatrixIndex firstObservationIndex){
