...
fName(name),
fProvisionalValue(NO_VALf,  NO_VALf,  NO_VALf,TCoordSysFactory::ECoordSys::k3DCartesian),
...
TReal TAdjustablePoint::getStandDev(int d)const{
	assert3D(d);
	if(!_isnan( fStandardDeviations[d]))
	...
	for (int i = 0; i < 3; i++){
		if (uidx[i] == idx) {
			if (i == 0 ){
				...
				fXValueSet = true;
				std::cout<< "point X"<<idx<<": "<<fEstimatedValue.getX().getValue()<<std::endl;
				...
				fYValueSet = true;
				std::cout<< "point Y"<<idx<<": "<<fEstimatedValue.getY().getValue()<<std::endl;
			}
			else{
				fCorrection.setZ(value);
				fEstimatedValue.setZ(fEstimatedValue.getZ()+value);
				std::cout<< "point Z"<<idx<<": "<<fEstimatedValue.getZ().getValue()<<std::endl;
			}
