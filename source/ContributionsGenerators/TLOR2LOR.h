...
class TLOR2LOR{
	public:
		/*!
			Structure keeping pointer to an adjustable transformation (adjTrafo) stored in a node of a tree, then transformation (trafo) between nodes which provides partial derivatives.
		...
		*/
		struct TransformAndParams{
			...
			bool direct;	// If TRUE it is a direct transformation, FALSE if inverse 
		};
		...
		*/
		...
		/*!
			Updates the transformation chain (transformationChain) and the overall transformation (transfo). 
	...
	private:
		...
		/*! Returns the comulative matrix which should multiply from left the matrix of partial derivatives (pre multiplication)*/
