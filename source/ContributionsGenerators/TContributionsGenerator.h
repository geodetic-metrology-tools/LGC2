/*! 
	...
	of the observations processed by LGC2
...
public:
	...
	*/ 
	...
	/*!@name Contributions to the total station (TSTN) observations. */
	//@{
	...
	/*!@name Contributions to the CAMERA(CAM) observations. */
	//@{

		///Returns contribution for a CAM's UVEC measurement
		UVECContrib	getUVECContrib(const TCAM& camera, const TUVEC& uvec);

		///Returns contribution for a CAM's UVD measurement 
		UVDContrib	getUVDContrib(const TCAM& camera, const TUVD& uvd);

	//@}

	/*!@name Contributions to the non TSTN and non CAMERA observations. */
	//@{

		///Returns contribution for a DVER measurement
		...
		///Returns contribution for a free spatial distance measurement (DSPT) made by EDM instrument
...
private:
	...
	//Name of the last processed stationed point, if the same as current in a call of function of CG, transformation to MLA does not need to be reinitialized
	...
	//@{
		...
		// Add contribution of a LOR transformations for 1D measurements (ANGL,ZEND,DIST,DHOR) into a 'transfContrib' vector, a,b,c are the coeficcients (see documentation in Mathematical Obsevation Models)
		...
		//Add contribution of a every target's LOR transformation in 'lorTrafo' into a 'transfContrib' vector for UVD measurement
		...
		TFreeVector getPointContributionDVER(const TLOR2LOR& lorTrafo, const  TPositionVector& cgrfPoint, bool station);
	//@}
