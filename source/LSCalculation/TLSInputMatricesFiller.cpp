		...
		for (TDataTreeIterator itTree = projData->getTree().begin(); itTree != projData->getTree().end(); itTree++){		
			...
			for(auto itCAM(itTree.node->data->measurements.fCAM.begin()); itCAM != itTree.node->data->measurements.fCAM.end(); ++itCAM){
				for(auto itUVD(itCAM->measUVD.begin()); itUVD != itCAM->measUVD.end(); ++itUVD)
						addUVDContribution(*itUVD, *itCAM, matrices);

				for(auto itUVEC(itCAM->measUVEC.begin()); itUVEC != itCAM->measUVEC.end(); ++itUVEC)
						addUVECContribution(*itUVEC, *itCAM, matrices);
			}
			...
			for(auto itEDM(itTree.node->data->measurements.fEDM.begin()); itEDM != itTree.node->data->measurements.fEDM.end(); ++itEDM){
				...
				for(auto itDPST(itEDM->measDSPT.begin()); itDPST != itEDM->measDSPT.end(); ++itDPST){	
					addDSPTContribution(*itDPST, *itEDM, matrices);
				}
			}
			...
			for(auto itDVER(itTree.node->data->measurements.fDVER.begin()); itDVER != itTree.node->data->measurements.fDVER.end(); ++itDVER)
				addDVERContribution(*itDVER, matrices);
		}
	}
...
void  TLSInputMatricesFiller::addLevelStContributions(const TLEVEL& levelSt, TLSInputMatrices*  matrices){
	for(auto itDLEV(levelSt.measDLEV.begin()); itDLEV != levelSt.measDLEV.end(); ++itDLEV){
		...
		// Set Misclosure vector
		isProcessOK = isProcessOK && matrices->setMisclosureVectorElement(eqIdx,  -1.0 * (itDLEV->getScalar() - contributions.fCalcMeas));  
	...
	for(auto itDHOR(levelSt.measDHOR.begin()); itDHOR != levelSt.measDHOR.end(); ++itDHOR){
		...
		if(!itDHOR->targetPos->isFixed())
			isProcessOK = isProcessOK && addPointContribution(*itDHOR->targetPos, contributionsDHOR.fStaffContrib, eqIdxHd, matrices);
		...
		if(!_isnan(itDHOR->getDHORSigma())){
			isProcessOK = isProcessOK && matrices->setWeightMtrxElement(obsIdxHd, obsIdxHd,1/pow2q(itDHOR->getDHORSigma()));
			isProcessOK = isProcessOK && matrices->setWeightInvMtrxElement(obsIdxHd, obsIdxHd, pow2q(itDHOR->getDHORSigma()));
		}
		else{
			//Throw exception or set some typical sigma, but we should force user to provide it
			throw std::runtime_error("If DHOR defined, standard deviation should be provided!");
		}
...
/*		//Statistical model needs to be reviewed
...
void TLSInputMatricesFiller::addUVDContribution(const TUVD& meas, const TCAM& camera, TLSInputMatrices*  matrices){
	//Get the observation contribution
	UVDContrib contributions = fCGenerator.getUVDContrib(camera, meas);
	bool isProcessOK = true;
	//Get the first and last equation index
	MatrixIndex firstEqIdx = meas.getFirstEquationIndex();
	MatrixIndex firstObsIdx = meas.getFirstObservationIndex();
	...
	if(!cameraPos->isFixed()){
		if(!cameraPos->isCoordinateFixed(0)){
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx, cameraPos->getCoordinateUnknIndex(0), contributions.fStCoordContrib.firstEqPtContrib.getX().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx + 1, cameraPos->getCoordinateUnknIndex(0), contributions.fStCoordContrib.secondEqPtContrib.getX().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx + 2, cameraPos->getCoordinateUnknIndex(0), contributions.fStCoordContrib.thirdEqPtContrib.getX().getValue());
		}
		if(!cameraPos->isCoordinateFixed(1)){ 
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx, cameraPos->getCoordinateUnknIndex(1), contributions.fStCoordContrib.firstEqPtContrib.getY().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx+1, cameraPos->getCoordinateUnknIndex(1), contributions.fStCoordContrib.secondEqPtContrib.getY().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx+2, cameraPos->getCoordinateUnknIndex(1), contributions.fStCoordContrib.thirdEqPtContrib.getY().getValue());
		}
		if(!cameraPos->isCoordinateFixed(2)){
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx, cameraPos->getCoordinateUnknIndex(2), contributions.fStCoordContrib.firstEqPtContrib.getZ().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx+1, cameraPos->getCoordinateUnknIndex(2), contributions.fStCoordContrib.secondEqPtContrib.getZ().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx+2, cameraPos->getCoordinateUnknIndex(2), contributions.fStCoordContrib.thirdEqPtContrib.getZ().getValue());
		}
	}
	...
	if(!targetPos->isFixed()){
		if(!targetPos->isCoordinateFixed(0)){
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx, targetPos->getCoordinateUnknIndex(0), contributions.fTgCoordContrib.firstEqPtContrib.getX().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx + 1, targetPos->getCoordinateUnknIndex(0), contributions.fTgCoordContrib.secondEqPtContrib.getX().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx + 2, targetPos->getCoordinateUnknIndex(0), contributions.fTgCoordContrib.thirdEqPtContrib.getX().getValue());
		}
		if(!targetPos->isCoordinateFixed(1)){ 
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx, targetPos->getCoordinateUnknIndex(1), contributions.fTgCoordContrib.firstEqPtContrib.getY().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx+1, targetPos->getCoordinateUnknIndex(1), contributions.fTgCoordContrib.secondEqPtContrib.getY().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx+2, targetPos->getCoordinateUnknIndex(1), contributions.fTgCoordContrib.thirdEqPtContrib.getY().getValue());
		}
		if(!targetPos->isCoordinateFixed(2)){
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx, targetPos->getCoordinateUnknIndex(2), contributions.fTgCoordContrib.firstEqPtContrib.getZ().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx+1, targetPos->getCoordinateUnknIndex(2), contributions.fTgCoordContrib.secondEqPtContrib.getZ().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx+2, targetPos->getCoordinateUnknIndex(2), contributions.fTgCoordContrib.thirdEqPtContrib.getZ().getValue());
		}
	}
...
void TLSInputMatricesFiller::addUVECContribution(const TUVEC& meas, const TCAM& camera, TLSInputMatrices*  matrices){
	//Get the observation contribution
	UVECContrib contributions = fCGenerator.getUVECContrib(camera, meas);
	bool isProcessOK = true;
	//Get the first and last equation index
	MatrixIndex firstEqIdx = meas.getFirstEquationIndex();
	MatrixIndex firstObsIdx = meas.getFirstObservationIndex();
	...
	if(!cameraPos->isFixed()){
		if(!cameraPos->isCoordinateFixed(0)){
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx, cameraPos->getCoordinateUnknIndex(0), contributions.fStCoordContribFirstEq.getX().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx + 1, cameraPos->getCoordinateUnknIndex(0), contributions.fStCoordContribSecondEq.getX().getValue());
		}
		if(!cameraPos->isCoordinateFixed(1)){ 
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx, cameraPos->getCoordinateUnknIndex(1), contributions.fStCoordContribFirstEq.getY().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx + 1, cameraPos->getCoordinateUnknIndex(1), contributions.fStCoordContribSecondEq.getY().getValue());
		}
		if(!cameraPos->isCoordinateFixed(2)){
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx, cameraPos->getCoordinateUnknIndex(2), contributions.fStCoordContribFirstEq.getZ().getValue());
			isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(firstEqIdx + 1, cameraPos->getCoordinateUnknIndex(2), contributions.fStCoordContribSecondEq.getZ().getValue());
		}
	}
...
void  TLSInputMatricesFiller::addDSPTContribution(const TDSPT& meas, const TEDM& edmST, TLSInputMatrices*  matrices){
	MatrixIndex eqIdx = meas.getFirstEquationIndex(); 
	MatrixIndex obsIdx = meas.getFirstObservationIndex(); 
	bool isProcessOK = true;

	DistMeasContrib contributions = fCGenerator.getDSPTContrib(edmST, meas); //Get the observation contribution
	...
	// Adding instrument height contribution
	if(!edmST.instrumentHeightAdjustable->isFixed())
		isProcessOK = isProcessOK && matrices->setFirstDgnMtrxElement(eqIdx, edmST.instrumentHeightAdjustable->getFirstUidx(), contributions.fHIContrib);
...
#if 1
void  TLSInputMatricesFiller::addDVERContribution(const TDVER& meas, TLSInputMatrices*  matrices){
	MatrixIndex eqIdx = meas.getFirstEquationIndex(); 
	MatrixIndex obsIdx = meas.getFirstObservationIndex(); 
	bool isProcessOK = true;

	DVERContrib contributions = fCGenerator.getDVERContrib(meas); //Get the observation contribution
	...
	// Add target's contributions into a first design matrix
	if(!meas.target->isFixed())
		isProcessOK = isProcessOK && addPointContribution(*meas.target, contributions.fTgCoordContrib, eqIdx, matrices);
	...
	// Set Misclosure vector
	isProcessOK = isProcessOK && matrices->setMisclosureVectorElement(eqIdx,  -1.0 * (meas.getObservedValue() - contributions.fCalcMeas));
...
bool	TLSInputMatricesFiller::fillWeightUnkMtrx(TLGCData* projData, TLSInputMatrices* matrices){
	...
	for (auto it = projData->getPoints().objects.begin(); it != projData->getPoints().objects.end(); ++it){	
