...
private:
	...
	//@{
		...
		void extractAngleObs(const TLSResultsMatrices& rm, TAMeas<TInstrumentData::TPOLAR::TTarget, ENoValues, 0, ESingleValue, 1>& anglMeas);

		/*!Extract the results relative to any distance measurement (DIST, DHOR) observation and set the calculated residual*/
		void extractDistObs(const TLSResultsMatrices& rm, TAMeas<TInstrumentData::TPOLAR::TTarget>& distanceMeas);

		/*!Extract the results relative to a PLR3D measurement observation and set the calculated residual*/
		void extractPLR3DObs(const TLSResultsMatrices& rm, TAMeas<TInstrumentData::TPOLAR::TTarget, ESingleValue, 1, EPLR3DAngles,  2>& plr3DMeas);

		/*!Extract the results relative to a DIR3D measurement observation and set the calculated residual*/
		void extractDIR3DObs(const TLSResultsMatrices& rm, TAMeas<TInstrumentData::TPOLAR::TTarget, ESingleValue, 0, EPLR3DAngles,  2>& dir3DMeas);
