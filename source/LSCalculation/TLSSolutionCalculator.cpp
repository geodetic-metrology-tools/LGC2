...
TLSSolutionCalculator::TLSSolutionCalculator() : fNumberOfIterations(0), fS0APosterioriVariances(false){
}


bool	TLSSolutionCalculator::iterate2Solution(TLGCData& data, int fMaxIterations, TReal convCrit){
	...
	while(!lastIteration && fNumberOfIterations < fMaxIterations)
	{
		...
		{
			// compute solution 
			bool computationOK = computer->computeResults(inputMtr.get(), resultMatrices.get());
	
			if (computationOK)
			{	
				bool extractOK = false;
				extractOK = extractor->extractResults(*resultMatrices.get(), convCrit);
				if (extractOK)
	...
	{
		computeVarCovarAndReliability(&data,inputMtr.get(),computer.get(),extractor.get(),resultMatrices.get());
		return true;
	}
}

void	TLSSolutionCalculator::computeVarCovarAndReliability(TLGCData* data,
