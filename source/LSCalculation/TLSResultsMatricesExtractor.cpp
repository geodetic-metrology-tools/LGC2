...
#include "Measurements\TUVEC.h"
#include "Measurements\TUVD.h"
...
void TLSResultsMatricesExtractor::extractDIR3DObs(const TLSResultsMatrices& rm, TAScalarMeas<TInstrumentData::TPOLAR::TTarget, ESingleValue, 0, EPLR3DAngles,  2>& dir3DMeas){
...
void TLSResultsMatricesExtractor::extractUVDObs(const TLSResultsMatrices& rm, TUVD& uvdMeas){
	...
	if ( sDistIDX < rm.getResidualsVctr()->size() ) 
		uvdMeas.setDistanceResidual(rm.getResidualsVctrElmt(sDistIDX));
	else
		throw std::runtime_error("UVD observation, problem during extraction residuals: observation index exceeds matrix dimensions");
}
...
void TLSResultsMatricesExtractor::extractLEVELObs(const TLSResultsMatrices& rm, TLEVEL& levelMeas){
	for(auto itDLEV(levelMeas.measDLEV.begin()); itDLEV != levelMeas.measDLEV.end(); ++itDLEV){
		...
		if ( obsUidx < rm.getResidualsVctr()->size() ) 
			itDLEV->setScalarResidual(rm.getResidualsVctrElmt(obsUidx));
		else
...
bool TLSResultsMatricesExtractor::extractPointParams(const TLSResultsMatrices& rm, const TReal convCrit)  {
	...
	for (auto it = fDataSet->getPoints().objects.begin(); it != fDataSet->getPoints().objects.end(); ++it){
		if(it->hasVariable()){
...
bool TLSResultsMatricesExtractor::extractAngleParams(const TLSResultsMatrices& rm, const TReal convCrit) {
	...
	for (auto it = fDataSet->getAngles().objects.begin(); it != fDataSet->getAngles().objects.end(); ++it){
		if(!it->isFixed()){
...
bool TLSResultsMatricesExtractor::extractPlaneParams(const TLSResultsMatrices& rm, const TReal convCrit){
	...
	for (auto it = fDataSet->getPlanes().objects.begin(); it != fDataSet->getPlanes().objects.end(); ++it){
		if(it->hasVariable()){
			for(int unknIdx = it->getFirstUidx(); unknIdx <= it->getLastUidx(); unknIdx++){
					if (unknIdx >= rm.getSolutionVctr()->size())
...
bool TLSResultsMatricesExtractor::extractScalarParams(const TLSResultsMatrices& rm, const TReal convCrit){
	bool critNotExceeded = true;
	for (auto it = fDataSet->getScalars().objects.begin(); it != fDataSet->getScalars().objects.end(); ++it){
		if(!it->isFixed()){
				MatrixIndex unknIdx = it->getFirstUidx();	//first=last only one unknown fo angle class
