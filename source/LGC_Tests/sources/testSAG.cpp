...
namespace tut
{
	struct test_SAG
	{
		...
		void runDeformExpansionChecks(const char *inputData, const std::string &testLabel)
		{
			...
			ensure_equals(testLabel + ": Vertical sag must be 0", testSag.getVertSag().getEstimatedValue().getMetresValue(), 0.0, 1e-9);
			ensure_equals(testLabel + ": Vertical curvature must be -1", testSag.getVertCurv().getEstimatedValue().getMetresValue(), -1.0, 1e-9);
			...
			{
				ensure(testLabel + ": Ref point should end with _ref suffix",
					pair.refPoint.size() > 4 && pair.refPoint.substr(pair.refPoint.size() - 4) == "_ref");
				ensure(testLabel + ": Assoc point should not have _ref suffix",
					pair.assocPoint.find("_ref") == std::string::npos);
				ensure(testLabel + ": Reference point should exist", projTest->getPoints().doesObjectExist(pair.refPoint));
				ensure(testLabel + ": Associated point should exist", projTest->getPoints().doesObjectExist(pair.assocPoint));
			}
		}
	};
...
namespace tut
{
	...
	void object::test<1>()
	{
		...
		ensure_equals("VS freedom not assigned correctly", firstSag.getVertSag().isFixed(), expected0.test(0));
		ensure_equals("VC freedom not assigned correctly", firstSag.getVertCurv().isFixed(), expected0.test(1));
		ensure_equals("RS freedom not assigned correctly", firstSag.getRadSag().isFixed(), expected0.test(2));
		ensure_equals("RC freedom not assigned correctly", firstSag.getRadCurv().isFixed(), expected0.test(3));
		ensure_equals("wrong number of unknowns in sag element", firstSag.getNumUnkn(), 4);
		// sag1 has a different fixed state signature (VS and RC free)
		std::bitset<4> expected1("0110");
		ensure_equals("VS freedom not assigned correctly", secondSag.getVertSag().isFixed(), expected1.test(0));
		ensure_equals("VC freedom not assigned correctly", secondSag.getVertCurv().isFixed(), expected1.test(1));
		ensure_equals("RS freedom not assigned correctly", secondSag.getRadSag().isFixed(), expected1.test(2));
		ensure_equals("RC freedom not assigned correctly", secondSag.getRadCurv().isFixed(), expected1.test(3));
		...
		ensure_equals("VS index not assigned correctly", firstSag.getVertSag().getFirstUidx(), firstIdx + 0);
		ensure_equals("VC index not assigned correctly", firstSag.getVertCurv().getFirstUidx(), firstIdx + 1);
		ensure_equals("RS index not assigned correctly", firstSag.getRadSag().getFirstUidx(), firstIdx + 2);
		ensure_equals("RC index not assigned correctly", firstSag.getRadCurv().getFirstUidx(), firstIdx + 3);
		...
		ensure_equals("VS index not assigned correctly", secondSag.getVertSag().getFirstUidx(), firstIdx + 0);
		try
		{
		...
		ensure_equals("RC index not assigned correctly", secondSag.getRadCurv().getFirstUidx(), firstIdx + 1);
	}
	...
	void object::test<2>()
	{
		...
		currentEst << sagObject.getVertSag().getEstimatedValue(), sagObject.getVertCurv().getEstimatedValue(),
		...
		updatedEst << sagObject.getVertSag().getEstimatedValue(), sagObject.getVertCurv().getEstimatedValue(),
			sagObject.getRadSag().getEstimatedValue(), sagObject.getRadCurv().getEstimatedValue();
	...
	void object::test<5>()
	{
		set_test_name("Testing estimation of sag params and y offset with fixed bearing");
		...
		ensure_equals("Radial sag must be 0.5m", testSag.getRadSag().getEstimatedValue().getMetresValue(), 0.5, 1e-9);
		ensure_equals("Radial curvature must be 1", testSag.getRadCurv().getEstimatedValue().getMetresValue(), 1.0, 1e-9);
		// sagFrame RZ is fixed at 19 gon (matching giveBearingAndTranslationFrame), only TY is estimated
	...
	void object::test<8>()
	{
		set_test_name("Testing point-level DEFORM syntax with point sigma transfer to _ref points");
	...
	void object::test<9>()
	{
		set_test_name("Testing frame-level DEFORM syntax with point sigma transfer to _ref points");
	...
	void object::test<10>()
	{
		set_test_name("Testing vertical sag with 2D points identifies frame rotation");

		projTest->getFileLogger().setOutputfileLocation("C:/Temp/SAG.txt");
		std::stringstream infiler(Sag::SAG_test_verticalWithRotation);
		projTest->getFileLogger().writeReportHeader("LGC output file");
