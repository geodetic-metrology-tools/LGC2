...
struct test_SAG
{
	...
	void runDeformExpansionChecks(const char *inputData, const std::string &testLabel)
	{
		...
		{
			ensure(testLabel + ": Ref point should contain _ref_<sagname> marker", pair.getRefPoint().find("_ref_") != std::string::npos);
			ensure(testLabel + ": Assoc point should not contain _ref_ marker", pair.getAssocPoint().find("_ref_") == std::string::npos);
...
void object::test<2>()
{
	...
	currentEst << sagObject.getZSag().getEstimatedValue(), sagObject.getZCurv().getEstimatedValue(), sagObject.getXSag().getEstimatedValue(),
		sagObject.getXCurv().getEstimatedValue();
	...
	updatedEst << sagObject.getZSag().getEstimatedValue(), sagObject.getZCurv().getEstimatedValue(), sagObject.getXSag().getEstimatedValue(),
		sagObject.getXCurv().getEstimatedValue();
