...
struct SubframeTestParams {
	...
	int frameIterations;
	const char* measurementType;  // "INCLY" or "ROLLY"
...
void runSubframeTest(const SubframeTestParams& params, std::shared_ptr<TLGCData> projTest, TReader& r) {
	...
	std::string coordMsg = std::string(params.pointName) + " x coordinate should match";
	ensure_equals(coordMsg.c_str(), point.getX().getMetresValue(), params.expectedX, params.coordinateTolerance);
	
	coordMsg = std::string(params.pointName) + " y coordinate should match";
	ensure_equals(coordMsg.c_str(), point.getY().getMetresValue(), params.expectedY, params.coordinateTolerance);
	
	coordMsg = std::string(params.pointName) + " z coordinate should match";
	...
	if (std::string(params.measurementType) == "INCLY") {
	...
	} else if (std::string(params.measurementType) == "ROLLY") {
 ...
 */
struct EnhancedKeywordValidationTestParams {
	const char* testData;
	const char* logFileName;
	const char* testDescription;
...
/*
 * Helper function for testing INCLY/ROLLY keyword parsing and parameter validation
 * 
 * This function validates the parsing and parameter inheritance for INCLY/ROLLY measurements,
 * ensuring that all keywords (INSTR, OBSE, PPM, AC, ACSE, RF, RFSE, ID) are correctly
 * parsed and applied to measurements. It follows the same pattern as DLEV keyword testing.
...
// Enhanced parameterized validation function - works for both INCLY and ROLLY
template<typename MeasurementContainer>
void validateMeasurementParameters(const MeasurementContainer& measurements, const std::vector<EnhancedKeywordValidationTestParams::MeasurementParams>& expectedMeasurements) {
...
void runEnhancedKeywordValidationTest(const EnhancedKeywordValidationTestParams& params, std::shared_ptr<TLGCData> projTest, TReader& r) {
	projTest->getFileLogger().setOutputfileLocation(params.logFileName);
	projTest->getFileLogger().writeReportHeader("LGC output file");
...
void runKeywordValidationTest(const KeywordValidationTestParams& params, std::shared_ptr<TLGCData> projTest, TReader& r) {
	...
	EnhancedKeywordValidationTestParams enhancedParams;
	enhancedParams.testData = params.testData;
	enhancedParams.logFileName = params.logFileName;
	enhancedParams.testDescription = params.testDescription;
	
	// Convert MeasurementParams vector - copy each element and set combinedSigma to 0.0
	enhancedParams.expectedMeasurements.clear();
	for (const auto& oldParam : params.expectedMeasurements) {
		EnhancedKeywordValidationTestParams::MeasurementParams newParam;
		newParam.instrumentID = oldParam.instrumentID;
		newParam.obse = oldParam.obse;
		newParam.ppm = oldParam.ppm;
		newParam.ac = oldParam.ac;
		newParam.acse = oldParam.acse;
		newParam.rf = oldParam.rf;
		newParam.rfse = oldParam.rfse;
		newParam.obsID = oldParam.obsID;
		newParam.measurementValue = oldParam.measurementValue;
		newParam.combinedSigma = 0.0; // Set to 0.0 for backward compatibility
		enhancedParams.expectedMeasurements.push_back(newParam);
	}
	
	runEnhancedKeywordValidationTest(enhancedParams, projTest, r);
}
...
void runINSTRKeywordTest(const INSTRTestParams& params, std::shared_ptr<TLGCData> projTest, TReader& r) {
	...
	if (std::string(params.measurementType) == "ROLLY") {
		rollyMeasurements = &frameIt.node->data->measurements.fROLLY;
	} else if (std::string(params.measurementType) == "INCLY") {
	...
	if (rollyMeasurements && !rollyMeasurements->empty()) {
		...
		for (size_t i = 0; i < params.expectedInstrumentIDs.size() && obsIt != rollyMeasurements->begin()->measROLLY.end(); ++i, ++obsIt) {
			std::string errorMsg = std::string(params.measurementType) + " instrument for measurement " + std::to_string(i + 1) + " should be " + params.expectedInstrumentIDs[i];
...
void object::test<23>()
{
	...
	std::vector<EnhancedKeywordValidationTestParams::MeasurementParams> expectedMeasurements = {
	...
	EnhancedKeywordValidationTestParams params = {
	...
	runEnhancedKeywordValidationTest(params, projTest, r);
}
