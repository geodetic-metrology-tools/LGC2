...
void object::test<3>()
{
	...
	ensure_equals("Fault detection quantiles must match default values", cfg.faut.alpha, 0.01);
	ensure_equals("Fault detection quantiles must match default values", cfg.faut.beta, 0.1);
	f1.parse(tokenizefileString("*FAUT 0.1 0.3"), true, -1);
	ensure_equals("Fault detection quantiles must match input", cfg.faut.alpha, 0.1);
	ensure_equals("Fault detection quantiles must match input", cfg.faut.beta, 0.3);
	//
	TKeyPUNC f2(proj);
	...
	ensure_equals("Using plain PUNC file format", cfg.writePunch.fmode, TLGCConfig::TCoordOut::kPLAIN);
	f2.parse(tokenizefileString("*PUNC T"), true, -1);
	ensure_equals("Using T PUNC file format", cfg.writePunch.fmode, TLGCConfig::TCoordOut::kT);
	//
	TKeySOBS f3(proj);
...
void object::test<6>()
{
	...
	// ANGL
	TKeyANGL ang(proj);
	ang.parse(tokenizefileString("*ANGL"), true, -1);
	...
	// DSPT
	TKeyDSPT dspt(proj);
	dspt.parse(tokenizefileString("*DSPT P1 DM1 IH 60 IHSE 61 ICSE 62"), true, -1);
	dspt.parse(tokenizefileString("P2 63 OBSE 64 PPM 65 TH 66 THSE 67 TCSE 68"), true, -1);
	...
	// ECSP
	TKeyCALA pr1(proj);
	pr1.parse(tokenizefileString("A 1 2 3"), true, -1);
