...
void object::test<4>()
{
	...
	TPositionVector PTRP = dataset.getPoints().getObject("DLEV_l17_RP").getEstimatedValue();
...
void object::test<5>()
{
	...
	TPositionVector PTRP = dataset.getPoints().getObject("DLEV_l21_RP").getEstimatedValue();
	ensure_equals("PT x coordinate should match", PTRP.getX().getMetresValue(), 40, 1e-7);
	ensure_equals("PT y coordinate should match", PTRP.getY().getMetresValue(), 34.0, 1e-7);
