...
void TSCALEWriter::writeECHOResults(const  TECHOROM& echorom)
{
	...
	(*stream).writeDouble(obsWidth,obsWidth, echorom.fMeasuredPlane->getReferencePoint()->getEstValue(0).getValue());
	(*stream).writeDouble(obsWidth,obsWidth, echorom.fMeasuredPlane->getReferencePoint()->getEstValue(1).getValue());
	(*stream).writeDouble(obsWidth,obsWidth, echorom.fMeasuredPlane->getReferencePoint()->getEstValue(2).getValue());
	(*stream)<<endl<<endl;
	///////////////////////////////////////////////////////////////////////////////////
	...
	for(auto const& ItECHO : echorom.measECHO)
	{
		...
		(*stream).writeDouble(obsResWidth, lengthResPrecision, ItECHO.target.sigmaD * LGC::M2MM);//Output value in meters [mm], stored in [m]
		...
		//write the residual
		(*stream).writeDouble(obsResWidth, lengthResPrecision, ItECHO.getDistanceResidual() * LGC::M2MM);//Output value in meters [mm], stored in [m]

		//write the residual/sigam
		(*stream).writeDouble(obsResWidth, lengthResPrecision, ItECHO.getDistanceResidual()/ItECHO.target.sigmaD);//Output value in meters [mm], stored in [m]
...
void	TSCALEWriter::writeECHOReliabilityData(const  TECHOROM& echorom, const TLGCStatistic& stat, const std::vector<TECHO> measECHO)
{
	...
	for(auto const& ItEcho : measECHO)
	{
		...
		(*stream).writeDouble(obsResWidth, lengthResPrecision, ItEcho.target.sigmaD* LGC::M2MM);
		//get the residual
