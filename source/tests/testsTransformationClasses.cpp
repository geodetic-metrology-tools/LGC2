...
namespace
{
    tut::factory tf("Test of transformation classes");
}
...
namespace tut
{
	...
	void object::test<1>()
	{
		...
		ensure("Estimated parameters should be equal", directTransfo.getTransformParam() == identity);
	}
	...
	void object::test<2>()
	{
		...
		ensure("Estimated parameters should be equal", inverseTransfo.getTransformParam() == identity);
	}
	...
	void object::test<3>()
	{
		...
		ensure("Estimated parameters should be equal", directTransfo.getTransformParam() == copy.getTransformParam());
	}
	...
	void object::test<4>()
	{
		...
		ensure("Estimated parameters should be equal", inverseTransfo.getTransformParam() == copy.getTransformParam());
	...
	void object::test<5>()
	{
		...
		directTransfo.setMatrix(param1);
		...
		ensure_distance("m(0,3)", directTransfo.getMmatrixIJPosition(0,3), param1.t1/param1.scale, 1e-6);
		...
		ensure_distance("m(1,3)", directTransfo.getMmatrixIJPosition(1,3), param1.t2/param1.scale, 1e-6);
		...
		ensure_distance("m(2,3)", directTransfo.getMmatrixIJPosition(2,3), param1.t3/param1.scale, 1e-6);
	...
	void object::test<6>()
	{
		...
		ensure_distance("m(0,3)", invTransfo.getMmatrixIJPosition(0,3), -(kappacos*phicos)*param1.t1 - (kappacos*phisin*omegasin-kappasin*omegacos)*param1.t2 - (kappacos*phisin*omegacos+kappasin*omegasin)*param1.t3, 1e-6);
		...
		ensure_distance("m(1,3)", invTransfo.getMmatrixIJPosition(1,3), -(kappasin*phicos)*param1.t1 - (kappasin*phisin*omegasin+kappacos*omegacos)*param1.t2 - (kappasin*phisin*omegacos-kappacos*omegasin)*param1.t3, 1e-6);
		...
		ensure_distance("m(2,3)", invTransfo.getMmatrixIJPosition(2,3), -(-phisin)*param1.t1 - (phicos*omegasin)*param1.t2 - (phicos*omegacos)*param1.t3, 1e-6);
	}
...
*/
	...
	{
		using namespace LGC;
		set_test_name("rotation partial derivative for a inverse transformation");
		...
		rotX(0,3) = -rotX(0,0)*param1.t1 - rotX(0,1)*param1.t2 - rotX(0,2)*param1.t3;
		rotX(1,1) = kappaSin*phiSin*omegaCos-kappaCos*omegaSin;
		rotX(1,2) = -kappaSin*phiSin*omegaSin-kappaCos*omegaCos;
		rotX(1,3) = -rotX(1,0)*param1.t1 - rotX(1,1)*param1.t2 - rotX(1,2)*param1.t3;
		rotX(2,1) = phiCos*omegaCos;
		rotX(2,2) = -phiCos*omegaSin;
		rotX(2,3) = -rotX(2,0)*param1.t1 - rotX(2,1)*param1.t2 - rotX(2,2)*param1.t3;		
		...
		rotY(0,3) = -rotY(0,0)*param1.t1 - rotY(0,1)*param1.t2 - rotY(0,2)*param1.t3;
		rotY(1,0) = -kappaSin*phiSin;
		rotY(1,1) = kappaSin*phiCos*omegaSin;
		rotY(1,2) = kappaSin*phiCos*omegaCos;
		rotY(1,3) = -rotY(1,0)*param1.t1 - rotY(1,1)*param1.t2 - rotY(1,2)*param1.t3;
		rotY(2,0) = -phiCos;
		rotY(2,1) = -phiSin*omegaSin;
		rotY(2,2) = -phiSin*omegaCos;
		rotY(2,3) = -rotY(2,0)*param1.t1 - rotY(2,1)*param1.t2 - rotY(2,2)*param1.t3;
		...
		rotZ(0,3) = -rotZ(0,0)*param1.t1 - rotZ(0,1)*param1.t2 - rotZ(0,2)*param1.t3;
		rotZ(1,0) = kappaCos*phiCos;
		rotZ(1,1) = kappaCos*phiSin*omegaSin-kappaSin*omegaCos;
		rotZ(1,2) = kappaCos*phiSin*omegaCos+kappaSin*omegaSin;
		rotZ(1,3) = -rotZ(1,0)*param1.t1 - rotZ(1,1)*param1.t2 - rotZ(1,2)*param1.t3;
	...
	{
		using namespace LGC;
		set_test_name("translation partial derivative for a inverse transformation");
	...
	{
		using namespace LGC;
		set_test_name("scale factor partial derivative for a inverse transformation");
		...
		ensure_distance("m(0,3)", derivTransfo.getMmatrixIJPosition(0,3), -(kappacos*phicos)*param1.t1 - (kappacos*phisin*omegasin-kappasin*omegacos)*param1.t2 - (kappacos*phisin*omegacos+kappasin*omegasin)*param1.t3, 1e-6);
		...
		ensure_distance("m(1,3)", derivTransfo.getMmatrixIJPosition(1,3), -(kappasin*phicos)*param1.t1 - (kappasin*phisin*omegasin+kappacos*omegacos)*param1.t2 - (kappasin*phisin*omegacos-kappacos*omegasin)*param1.t3, 1e-6);
		...
		ensure_distance("m(2,3)", derivTransfo.getMmatrixIJPosition(2,3), -(-phisin)*param1.t1 - (phicos*omegasin)*param1.t2 - (phicos*omegacos)*param1.t3, 1e-6);
	...
	void object::test<19>()
	{
		...
		transfo.setMatrix(param1);


		TPositionVector vector_to_transform(0, 0, 0, k3DCartesian);
		/*transfo.TTransformation::transform(vector_to_transform);
		int a=0;*/
