...
namespace tut
{
	...
	void object::test<1>()
	{
		...
		ensure("Input title must match extracted title.", proj->getConfig().title == title);
	}
	...
	void object::test<4>()
	{
		...
		*/
		...
		void object::test<5>()
		{
			...
			proj.getConfig().sim.ignoreMeasurements = false;
			using namespace LGC;
...
#if 0 
			//Need to be rewritten for the new observation types
			TKeyCAM cam(proj);
			...
			ensure_equals("Point coordinates should match", dverM.target->getProvisionalValue().getX().getValue(),1);
			...
			ensure_equals("Measured vertical distance should match", firstDLEVMeasurement.getScalar(),5);
			...
			ensure_equals("Measured vertical distance should match", firstDLEVMeasurement2.getScalar(),6);
