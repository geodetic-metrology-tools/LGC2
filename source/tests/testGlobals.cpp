...
namespace tut
{
	...
	void object::test<3>()
	{
		...
		ensure_equals("One instrument defined", proj5.instruments.fPOLAR.size(),1);
		ensure_equals("Provisional value of the pair should match", proj5.scalars.getObject("TS1T1").getProvisionalValue().getValue(),2.0);
		ensure_equals("Dist. correction is 2.0",(*proj5.instruments.fPOLAR["TS1"].targets["T1"].distCorrectionAdjustable).getProvisionalValue().getValue(),2.0);	
		ensure_equals("2 adjustable scalars expected",proj5.scalars.numObjects(), 9);
		
		TDataTreeIterator currentNodeIter = proj5.tree.begin();
		++currentNodeIter;
		auto& node(*(currentNodeIter->get()));
		ensure_equals("In the first node are 4 TSTN",node.measurements.fTSTN.size(),4);
		ensure_equals("Instrument height should be 0",(*node.measurements.fTSTN[0].instrumentHeightAdjustable).getProvisionalValue().getValue(),0.0);

		ensure_equals("Dist. correction should be 2.0",(*node.measurements.fTSTN[0].instrument.targets["T1"].distCorrectionAdjustable).getProvisionalValue().getValue(),2.0);
	}
	...
	void object::test<4>()
	{
		...
		inpMatFillTest.fOutputFileWriter.setOutputfileLocation("C:/Temp/output.txt");
		rr.read(infiler);
		...
		TLSInputMatricesFiller imf(&inpMatFillTest.tree, ct);
...
/*		//DIST
		ensure_equals("(0,0) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(0,0), 1.0, 1e-8);
		ensure_equals("(0,1) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(0,1), 0.0, 1e-8);
		ensure_equals("(0,2) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(0,2), 0.0, 1e-8);
		ensure_equals("(1,0) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(1,0), 1.0, 1e-8);
		ensure_equals("(1,1) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(1,1), 0.0, 1e-8);
		ensure_equals("(1,2) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(1,2), 0.0, 1e-8);
		ensure_equals("(2,0) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(2,0), 1.0, 1e-8);
		ensure_equals("(2,1) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(2,1), 0.0, 1e-8);
		ensure_equals("(2,2) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(2,2), 0.0, 1e-8);
		//ANGL
		ensure_equals("(3,0) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(3,0), 0.0, 1e-8);
		ensure_equals("(3,1) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(3,1), -0.1, 1e-8);
		ensure_equals("(3,2) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(3,2), -1.0, 1e-8);
		ensure_equals("(4,0) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(4,0), 0.0, 1e-8);
		ensure_equals("(4,1) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(4,1), -0.1, 1e-8);
		ensure_equals("(4,2) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(4,2), -1.0, 1e-8);

		ensure_equals("(5,0) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(5,0), 0.0, 1e-8);
		ensure_equals("(5,0) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(5,1), -0.1, 1e-8);
		ensure_equals("(5,0) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(5,2), -1.0, 1e-8);
		//ZEND
		ensure_equals("(6,0) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(6,0), 0.0, 1e-8);
		ensure_equals("(6,1) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(6,1), 0.0, 1e-8);
		ensure_equals("(6,2) element of First design matrix should match", inputMtr.getFirstDgnMtrx()->coeff(6,2), 0.0, 1e-8);
		*/

	//	ensure_equals("First element of misclosure vector should match", inputMtr.getMisclosureVctr().coeff(0), 0.0, 1e-8);
	//	ensure_equals("First element of misclosure vector should match", inputMtr.getMisclosureVctr().coeff(1), -0.1, 1e-8);
//

/*
		TLGCCalculation calcul(inpMatFillTest);
...
*/	
		...
		set_test_name("Testing Input matrices filler.");
		TReader rr(&inpMatFillTest);
		stringstream infiler(LGCTestInputCG::CGfile);
		inpMatFillTest.fOutputFileWriter.setOutputfileLocation("C:/Temp/output.txt");
		rr.read(infiler);
		...
		TLGCData inpMatFillTestMLA;
		set_test_name("Testing Input matrices filler, MLA version.");
		TReader rrMLA(&inpMatFillTestMLA);
		stringstream infilerMLA(LGCTestInputCGMLA::CGfileMLA);
		inpMatFillTestMLA.fOutputFileWriter.setOutputfileLocation("C:/Temp/outputMLA.txt");
		rrMLA.read(infilerMLA);

		TLGCCalculation calculMLA(inpMatFillTestMLA);
		calculMLA.computeLSResults();*/
	//}
/*	//*/
...
//
//		TContributionsGenerator cg(&project2.tree, cType.fReferential);
////DLEV measurement in ROOT
//		TLEVEL level1 = *project2.tree.begin()->get()->measurements.fLEVEL.begin();
//		TDLEV dlev1 = project2.tree.begin()->get()->measurements.fLEVEL.begin()->measDLEV[0];
...
//		TTSTN station = *project2.tree.begin()->get()->measurements.fTSTN.begin();
...
//		TTSTN stationTgLOR = project2.tree.begin()->get()->measurements.fTSTN[1];
...
//		TCAM cam1 = *project2.tree.begin()->get()->measurements.fCAM.begin();
...
//		TDataTreeIterator it = project2.tree.begin();
//		it++;
...
////FIRST measurement ROOT
//		TTSTN stationB = *project2b.tree.begin()->get()->measurements.fTSTN.begin();
