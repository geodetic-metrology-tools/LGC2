...
{	
	...
	{ 
		...
		set_test_name("SIMPLE DHOR TEST");
		TReader r(&projTest);
		projTest.getFileLogger().setOutputfileLocation("C:/Temp/DHORTest_1.txt");
		projTest.getFileLogger().writeReportHeader("LGC output file");
		
		stringstream infiler(TestROOT::DHORTest_1);
		...
		const TLGCData& dataset = calcul.getData();
		TPositionVector Pt = dataset.getPoints().getObject("PT").getEstimatedValue();
		ensure_equals("Pt x coordinate should match",Pt.getX().getValue(), 100, 1e-8);
		ensure_equals("Pt y coordinate should match",Pt.getY().getValue(), 0, 1e-8);
        ...
        ensure_equals("V0 calculation should match for total station ST2",V0Calc, 0.0, 1e-8); 
