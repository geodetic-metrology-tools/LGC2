...
void TKeyOLOC::parse(const std::vector<const std::string>&, int) 
{
...
void TKeyRS2K::parse(const std::vector<const std::string>&, int) 
{
...
void TKeyLEP::parse(const std::vector<const std::string>&, int) 
{
...
void TKeySPHE::parse(const std::vector<const std::string>&, int) {
...
void TKeyALLFIXED::parse(const std::vector<const std::string>&, int) {
	fconfig.allfixed = TLGCConfig::TBinaryOption(true);
}


void TKeyLIBR::parse(const std::vector<const std::string>&, int) {
	fconfig.libre = TLGCConfig::TBinaryOption(true);
}


void TKeyNODUP::parse(const std::vector<const std::string>&, int) {
	fconfig.nodup = TLGCConfig::TBinaryOption(true);
}


void TKeyPDOR::parse(const std::vector<const std::string>& tokens, int) {
...
void TKeySIMU::parse(const std::vector<const std::string>& tokens, int) {
	auto numTokens = tokens.size();

	if (numTokens == 3)
		fconfig.sim = TLGCConfig::TSimulation(std::stoi(tokens.at(2)), SIM_DEF_IGNORE_OBS);
	else if (numTokens == 4)
		if (tokens.at(3) == "OBS")
			fconfig.sim = TLGCConfig::TSimulation(std::stoi(tokens.at(2)), true);
		else if (tokens.at(3) == "NOBS")
			fconfig.sim = TLGCConfig::TSimulation(std::stoi(tokens.at(2)), false);
		else
			throw std::runtime_error("Option for *SIMU is either NOBS or OBS.");
	else
...
void TKeyAPRI::parse(const std::vector<const std::string>&, int) {
	fconfig.useApriori = TLGCConfig::TBinaryOption(true);
}


void TKeyEREL::parse(const std::vector<const std::string>& tokens, int) {
    auto numtokens(tokens.size());
 ...
 TKeyFMTxHelper::TKeyFMTxHelper(TLGCConfig::TCustomOutputSep& co, const std::vector<const std::string>& tokens) {
	auto numTokens = tokens.size();
...
void TKeyFMTO::parse(const std::vector<const std::string>& tokens, int) {
	TKeyFMTxHelper(fconfig.CustomOutputSeparator, tokens);
}


void TKeyFMTP::parse(const std::vector<const std::string>& tokens, int) {
	TKeyFMTxHelper(fconfig.CustomOutputSeparatorPunch, tokens);
}


void TKeyHIST::parse(const std::vector<const std::string>&, int) {
	fconfig.histo = TLGCConfig::TBinaryOption(true);
}

void TKeyPREC::parse(const std::vector<const std::string>& tokens, int) {
	if (tokens.size() != 3)
...
void TKeyPRES::parse(const std::vector<const std::string>&, int) {
...
void TKeyDEFA::parse(const std::vector<const std::string>&, int) {
	fconfig.writeDefa = TLGCConfig::TBinaryOption(true);
}


void TKeyFAUT::parse(const std::vector<const std::string>& tokens, int) {
	auto numTokens = tokens.size();
...
void TKeyPUNC::parse(const std::vector<const std::string>& tokens, int) {
	auto numtokens = tokens.size();
...
void TKeyPLOT::parse(const std::vector<const std::string>& tokens, int) {
	auto numtokens = tokens.size();
...
void TKeySOBS::parse(const std::vector<const std::string>& tokens, int) {
	auto numtokens = tokens.size();
