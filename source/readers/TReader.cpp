...
bool TReader::read(std::istream& lgcStream) {
	...
	string line;
	int nline(0);
	bool isReferebceSystemDefined = false;
	...
	for(auto currenthandler(lasthandler); 
		...
		if (tokLine[0] == "*")  {
			...
			try {
				...
				// abort if there is no last handler
				if(!lasthandler){
					outputMessages << TFileLogger::e_logType::LOG_ERROR << nlinestr + "There is no keyword before key:\"" + currentkey + ", at line "+line;
					continue;
				}
				...
				// Particular case : TSTN authorized after ENDFRAME if all frames are closed
				else 
