...
// Pasring stops on a comment sign, the comment is then stored as a single token including the comment character.
std::vector<const std::string> const TReader::tokenizeLGCfileString(const std::string& str) {
		using namespace std;
		enum {
			STATE_TOKEN,
			STATE_DELIM
...
TReader::TReader(TLGCData* proj):
	...
	TAKeyWord* dir3d = new TKeyDIR3D(project);
	TAKeyWord* cam  = new TKeyCAM(project);

	// plr3d and dir3d commented because they can occur either below v0 or below CAM, their occurrence is ambiguous
	tstn->addChild(v0);
	v0->addChild(angl);
	v0->addChild(zend);
	v0->addChild(dist);
...
//	TAKeyWord* cam  = new TKeyCAM(project);
/*	TAKeyWord* plr3dCam = new TKeyPLR3D(project);
	TAKeyWord* dir3dCam = new TKeyDIR3D(project);*/
/*	cam->addChild(plr3d);
	cam->addChild(dir3d);*/
/*	plr3dCam->setParent(cam);
	dir3dCam->setParent(cam);*/
//	finterpreters.emplace_back(UPK(cam));
	/*finterpreters.emplace_back(UPK(plr3dCam));
	finterpreters.emplace_back(UPK(dir3dCam));*/
	...
	finterpreters.emplace_back(UPK(dir3d));
	finterpreters.emplace_back(UPK(cam));
...
bool TReader::read(std::istream& lgcStream) {
	...
	auto& outputMessages(project.fOutputFileWriter);
	...
	project.cfg.title = line;
	// restore the asterisk that was gobbled up by getline
	lgcStream.putback('*');
	...
	for(auto currenthandler(lasthandler); 
		lgcStream.good() && getline(lgcStream, line) && (line != "*END" && line != "*FIN"); 
		++nline) {
		...
		if (tokLine[0] == "*")  {
			...
			// check if the selected keyword is allowed here
			else if ((currenthandler->hasParent() && 
				currenthandler->getParent() != lasthandler) && (lasthandler->hasParent() && currenthandler->getParent() != lasthandler->getParent())){
		...
		if (currenthandler == nullptr){
			outputMessages << TFileLogger::e_logType::LOG_ERROR << nlinestr + "No handler assigned to this line, check error above.";
		}
		else{
			try{
				...
				outputMessages << TFileLogger::e_logType::LOG_ERROR << nlinestr + excp.what(); // + "Type of error: " + typeid(excp).name();
			}
		}
