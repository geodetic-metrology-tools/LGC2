...
class TAKeyWord {
	public:
		// The keyword to which the object reacts, e.g, FAUT
		...
		* [*][DVER][P1][P2][5][/1.2][$ This is a  DB_COMMENT]
		*/
		virtual void parse(const std::vector<const std::string>& tokens, int line) = 0;
...
/*!
	Helps finding flags and options in tokenized lines. Provides functions to check for flags
	and extract parameters as a one-liner in the parsers.
*/
class TOptionHelper {

	public:
		TOptionHelper(std::vector<const std::string>::const_iterator begin,
	...
	private:
		std::vector<const std::string>::const_iterator fbegin;
		std::vector<const std::string>::const_iterator fend;

		std::vector<const std::string>::const_iterator get(const std::string& opt) const {
