...
bool TLGCCalculation::computeLSResults(){
	...
	TLSInputMatricesFiller matrFiller(&fData.getTree(), fCalculationType);
...
bool	TLGCCalculation::iterate2Solution(TLGCData& fData,
	...
	while((!lastIteration) && (fNumberOfMadeIterations < fMaxIterations) && (!fData.getFileLogger().hasErrors()))
	{
		...
		if (mtrFilled)
		{
			...
			else
			{
				//Write errors which occured in computer of LS methos
				 fData.getFileLogger().writeReportHeader("Problem with LS results\n");
				 fData.getFileLogger() << computer->getError();
				 return false;
			}
...
//	inputMtr->saveMatricesToFile(fNumberOfMadeIterations);
	...
	if(fData.fUEOIndices.UIndex != 0) // && fData.cfg.faut.isActive = true
	{
		...
		TReal alpha = fData.getConfig().faut.alpha/100;
		TReal beta = fData.getConfig().faut.beta/100;
