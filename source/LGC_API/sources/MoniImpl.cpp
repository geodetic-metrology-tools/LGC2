...
void Moni::MoniImpl::createMeasurementReferences()
{
	for (TDataTreeIterator itTree = project.get()->getTree().begin(); itTree != project.get()->getTree().end(); itTree++)
	{
		...
		for (auto &itECSProm : itTree.node->data->measurements.fECSP)
		{
			...
			{
				measRefs.ECSP.insert({itECSP.obsID, itECSP});
				measRefs.types.insert({itECSP.obsID, "ECSP"});
			}
		}
		...
		for (auto &itROLLYrom : itTree.node->data->measurements.fROLLY)
		{
			for (auto &itROLLY : itROLLYrom.measROLLY)
			{
				measRefs.ROLLY.insert({itROLLY.obsID, itROLLY});
				measRefs.types.insert({itROLLY.obsID, "ROLLY"});
			}
		}
...
void Moni::MoniImpl::updateMeas(const std::string &id, const Eigen::VectorXd &measurementVector)
{
	...
	{
		measRefs.ROLLY.at(id).setAngle(TAngle(measurementVector[0], TAngle::kGons));
		return;
	}
	else if (type == "ECWS")
	{
...
Eigen::VectorXd Moni::MoniImpl::getEstimateResidual(const std::string &id)
{
	...
	{
		Eigen::VectorXd res(1);
		res << measRefs.ROLLY.at(id).getAngleResidual();
		return res;
	}
	else if (type == "ECWS")
	{
...
Eigen::VectorXd Moni::MoniImpl::getObsSigma(const std::string &id)
{
	...
	{
		Eigen::VectorXd result(1);
		result[0] = measRefs.ROLLY.at(id).target.sigmaAngl.getRadiansValue();
		return result;
	}
	else if (type == "ECWS")
	{
...
Eigen::VectorXd Moni::MoniImpl::getMeas(const std::string &id)
{
	...
	{
		Eigen::VectorXd result(1);
		result[0] = measRefs.ROLLY.at(id).getAngle();
		return result;
	}
	else if (type == "ECWS")
	{
...
void Moni::MoniImpl::setObsSigma(const std::string &id, const Eigen::VectorXd &sigma)
{
	...
	{
		measRefs.ROLLY.at(id).target.sigmaAngl.setRadiansValue(sigma(0));
	}
	else if (type == "ECWS")
	{
