...
Eigen::VectorXd TMonitorImpl::getResidual(std::string id)
{
	// get measurement residual after adjustemnt
	// check if id exists
	if (measRefs.types.count(id) == 0)
	{
		std::cout << "No measurement with ID " << id << " found." << std::endl;
	...
	{
		Eigen::VectorXd result(1);
		result[0] = ((double) measRefs.ANGL.at(id).getAngleResidual());
		return result;
	}
	else if (type == "ZEND")
	{
		Eigen::VectorXd result(1);
		result[0] = ((double) measRefs.ZEND.at(id).getAngleResidual());
		return result;
	}
	else if (type == "DIST")
	{
		Eigen::VectorXd result(1);
		result[0] = ((double) measRefs.DIST.at(id).getDistanceResidual());
		return result;
	}
	else if (type == "ECTH")
	{
		Eigen::VectorXd result(1);
		result[0] = ((double) measRefs.ECTH.at(id).getDistanceResidual());
		return result;
	}
	else if (type == "ECDIR")
	{
		Eigen::VectorXd result(1);
		result[0] = ((double) measRefs.ECDIR.at(id).getDistanceResidual());
		return result;
	}
	else if (type == "DHOR")
	{
		Eigen::VectorXd result(1);
		result[0] = ((double) measRefs.DHOR.at(id).getDistanceResidual());
		return result;
	}
	else if (type == "PLR3D")
	{
		Eigen::VectorXd result(3);
		result << measRefs.PLR3D.at(id).getAngleResidual(kANGL), measRefs.PLR3D.at(id).getAngleResidual(kZEND), measRefs.PLR3D.at(id).getDistanceResidual();
		return result;
	}
	else if (type == "ORIE")
	{
		Eigen::VectorXd result(1);
		result << measRefs.ORIE.at(id).getAngleResidual();
		return result;
	}
	else if (type == "UVEC")
	{
	...
	{
		Eigen::VectorXd result(1);
		result[0] = measRefs.DSPT.at(id).getDistanceResidual();
		return result;
	}
	else if (type == "DLEV")
	{
		// ignoring DHOR
		Eigen::VectorXd result(1);
		result[0] = measRefs.DHOR.at(id).getDistanceResidual();
		return result;
	}
	else if (type == "ECHO")
	{
		Eigen::VectorXd result(1);
		result[0] = measRefs.ECHO.at(id).getDistanceResidual();
		return result;
	}
	else if (type == "ECSP")
	{
		Eigen::VectorXd result(1);
		result[0] = measRefs.ECSP.at(id).getDistanceResidual();
		return result;
	}
	else if (type == "ECVE")
	{
		Eigen::VectorXd result(1);
		result[0] = measRefs.ECVE.at(id).getDistanceResidual();
		return result;
	}
	else if (type == "INCLY")
	{
		Eigen::VectorXd result(1);
		result[0]=measRefs.INCLY.at(id).getAngleResidual();
		return result;
	}
	else if (type == "ECWS")
	{
		Eigen::VectorXd result(1);
		result[0] = measRefs.ECWS.at(id).getDistanceResidual();
		return result;
	}
	else if (type == "DVER")
	{
		Eigen::VectorXd result(1);
		result[0] = measRefs.DVER.at(id).getDistanceResidual();
		return result;
	}
	else if (type == "RADI")
	{
	...
	{
		Eigen::VectorXd result(3);
		result << (double)measRefs.OBSXYZ.at(id).getXResidual(), (double)measRefs.OBSXYZ.at(id).getYResidual(), (double)measRefs.OBSXYZ.at(id).getZResidual();
		return result;
	}
}
