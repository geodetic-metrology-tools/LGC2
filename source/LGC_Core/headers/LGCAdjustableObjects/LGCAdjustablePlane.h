...
public:
	...
	/*!
		...
		\note Reference point distance (refPointDistance) is always variable, this object therefore introduces from 1 up to 3 unknowns (up to 2 for the two angle THETA and
	   PHI + 1 RP distance). Reference point is adjusted separately from the LGCAdjustablePoint object independently on the plane.
	*/
	...
	*/
	inline virtual int getNumUnkn() const { return (int)!fThetaFixed + (int)!fPhiFixed + (int)!fRefPtDistFixed; }
	...
	*/
	...
	inline bool isPhiFixed() { return fPhiFixed; }

	///	Tells if the RefPtDist length is fixed or unknown.
	inline bool isRefPtDistFixed() { return fRefPtDistFixed; }
	...
	/// Returns index of an unknown index of the reference point for the RefPtDist length in LS matrices.
	inline int getRefPtDistUnknIndex() const
	{
		if (!fRefPtDistFixed)
			return uidx_rpDistance;
		throw std::logic_error("Trying to get unknown index from fixed RefPtDist length of the plane.");
	}
...
private:
	...
	bool fRefPtDistFixed; /*!< Tells if reference point distance is fixed or not.*/
