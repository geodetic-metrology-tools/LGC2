...
template<typename TTarget,
	...
	protected:
		...
		std::array<TLength, numAngles>		anglesResidualsOffsets;  // Corresponding residual offsets (taking into account the distance between instrument and target!)
	...
	public:
		...
		*/
		...
		/// Returns residual of the observed angle provided as a corresponding transversal offset (at the target level)
		const TLength& getAngleResidualOffset(TEnumAngle id = kValue) const {
			return anglesResidualsOffsets[id];
		}
		//@}
		...
		/// Sets a residual of observed angle: provides the transverse impact (as a distance offset) estimated at the target level
		void setAngleResidualOffset(const TLength& e, TEnumAngle id = kValue) {
			anglesResidualsOffsets[id] = e;
		}
		//@}
