...
public:
	...
	//@{
	/// Constructor
	TAKeyWord(const std::string &word, TLGCData &project) : key(word), fparent(nullptr), proj(project) {}
	...
	*/
	...
	{
		return std::find_if(fsiblings.cbegin(), fsiblings.cend(), [&](const TAKeyWord *kw) { return kw->key == keyword; }) != fsiblings.cend();
	}
	...
	inline bool hasParent() { return fparent != nullptr; }
...
{
public:
	TOptionHelper(std::vector<std::string>::const_iterator begin, std::vector<std::string>::const_iterator end) : fbegin(begin), fend(end) {}

	bool has(const std::string &opt) const { return std::find(fbegin, fend, opt) != fend; }
