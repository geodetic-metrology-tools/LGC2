...
void LGCFrameConstraintGroup::addFrameToGroup(std::string slave, const TLGCData& projData)
{
	...
	if (slaves.size() > 0)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			...
			{
				throw std::runtime_error("The freedoms of translation parameter " + std::to_string(j) + " of master frame " + masterName + " and slave frame " + slaveName + " in slave group " + groupName + "are incompatible.");
			}
		...
		{
			throw std::runtime_error("The freedoms of the scale parameter of master frame " + masterName + " and slave frame " + slaveName + " in slave group " + groupName + " are incompatible.");
		}
