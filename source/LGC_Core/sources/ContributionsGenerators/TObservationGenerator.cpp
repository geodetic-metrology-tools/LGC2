...
TReal TObservationGenerator::getZENDCalcMeas(const TTSTN &station, const LGCAdjustablePoint *targetAdjPoint, TReal targetHt)
{
	...
	TReal distance3D = dist3D(stationPos.getX().getMetresValue(), stationPos.getY().getMetresValue(),
		stationPos.getZ().getMetresValue() + station.instrumentHeightAdjustable->getEstimatedValue(), targetPos.getX().getMetresValue(),
		targetPos.getY().getMetresValue(), targetPos.getZ().getMetresValue() + targetHt);
	...
	return (TAngle::aCos(((targetPos.getZ().getMetresValue() + targetHt - stationPos.getZ().getMetresValue() - station.instrumentHeightAdjustable->getEstimatedValue()) / distance3D)))
		.getRadiansValue();
}
...
TReal TObservationGenerator::getDISTCalcMeas(const TTSTN &station, const LGCAdjustablePoint *targetAdjPoint, TReal targetHt, TReal distanceCorr)
{
	...
	const TLOR2LOR &tgLor2RootTrafo = fPointTransfo->getLORTransformation(
		targetAdjPoint->getFrameTreePosition(), fPointTransfo->getTree()->begin()); // Get transformation from "Target lor" to "ROOT"
	tgLor2RootTrafo.transform(targetPos);
	...
	return (dist3D(stationPos.getX().getMetresValue(), stationPos.getY().getMetresValue(),
				(stationPos.getZ().getMetresValue() + station.instrumentHeightAdjustable->getEstimatedValue()), targetPos.getX().getMetresValue(),
				targetPos.getY().getMetresValue(), (targetPos.getZ().getMetresValue() + targetHt))
		- distanceCorr);
}
...
TReal TObservationGenerator::getDHORCalcMeas(const TTSTN &station, const TLINE &dhor)
{
	...
	const TLOR2LOR &tgLor2RootTrafo = fPointTransfo->getLORTransformation(
		dhor.targetPos->getFrameTreePosition(), fPointTransfo->getTree()->begin()); // Get transformation from "Target lor" to "ROOT"
	tgLor2RootTrafo.transform(targetPos);
...
TReal TObservationGenerator::getECTHCalcMeas(const TTSTN &station, const TTSTN::TROM &rom, const TECTH &ecth)
{
	...
	const TLOR2LOR &tgLor2RootTrafo = fPointTransfo->getLORTransformation(
		ecth.targetPos->getFrameTreePosition(), fPointTransfo->getTree()->begin()); // Get transformation from "Target lor" to "ROOT"
	tgLor2RootTrafo.transform(targetPos);
...
{
	DLEVContrib contributions = fCGenerator.getDLEVContrib(levelInstr, dlev);
...
{
	HorDistContribLEVEL contributions = fCGenerator.getHorDistContrib(referencePoint, dhor);
	return contributions.fCalcMeas;
}
...
TReal TObservationGenerator::getDSPTCalcMeas(const TEDM &edmST, const TDSPT &dspt)
{
	...
	const TLOR2LOR &tgLor2RootTrafo = fPointTransfo->getLORTransformation(
		dspt.targetPos->getFrameTreePosition(), fPointTransfo->getTree()->begin()); // Get transformation from "Target lor" to "ROOT"
	tgLor2RootTrafo.transform(targetPos);
...
TFreeVector TObservationGenerator::getUVECCalcMeas(const TCAM &camera, const TUVEC &uvec)
{
	...
	TFreeVector deltaStTg((targetPos.getX() - cameraPos.getX()).getMetresValue(), (targetPos.getY() - cameraPos.getY()).getMetresValue(),
		targetPos.getZ().getMetresValue() - cameraPos.getZ().getMetresValue(), TCoordSysFactory::k3DCartesian);
...
TReal TObservationGenerator::getORIECalcMeas(const TORIEROM &orieROM, const TORIE &orie)
{
	...
	const TLOR2LOR &tgLor2RootTrafo = fPointTransfo->getLORTransformation(
		orie.targetPos->getFrameTreePosition(), fPointTransfo->getTree()->begin()); // Transformation from "TARGET FRAME" to "ROOT"
	tgLor2RootTrafo.transform(targetPos);
...
TReal TObservationGenerator::getINCLYCalcMeas(const TINCLYROM &inclyROM, const TINCLY &incly)
{
	...
	INCLContrib contributions = fCGenerator.getINCLYContrib(inclyROM, incly);
...
TReal TObservationGenerator::getROLLYCalcMeas(const TROLLYROM &rollyROM, const TROLLY &rolly)
{
	...
	INCLContrib contributions = fCGenerator.getROLLYContrib(rollyROM, rolly);
