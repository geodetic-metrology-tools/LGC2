...
void TDataAnalyzer::assignObsIndex(TMeas &meas)
{
	...
	fData.fUEOIndices.OIndex += meas.getObsDim();
}

template<typename TMeas>
void TDataAnalyzer::assignEqIndex(TMeas &meas)
{
	meas.setFirstEquationIndex(fData.fUEOIndices.EIndex);
			...
			if (!itLEVEL->fRefPt)
			{
				...
				if (numberOfMeasurements > 0)
				{
					...
					LGCAdjustablePoint fRefPt = LGCAdjustablePoint(
						stationPoint, false, false, !itLEVEL->ihfix, "DLEV_line" + std::to_string(itLEVEL->line), fData.getConfig().referential, fData.getConfig().geoid.type, fTree.begin());
		...
		for (auto itECHO(it.node->data.get()->measurements.fECHO.begin()); itECHO != it.node->data.get()->measurements.fECHO.end(); ++itECHO)
		{
			...
			if (numberOfMeasurements > 0)
			{
				...
				std::shared_ptr<LGCAdjustablePoint> refPoint = std::make_shared<LGCAdjustablePoint>(
					...
					"ECHO_line" + std::to_string(itECHO->line), fData.getConfig().referential, fData.getConfig().geoid.type, fTree.begin());
		...
		for (auto &itECWS : it.node->data.get()->measurements.fECWS)
		{
			...
			else
			{
				...
				for (auto &itECWSMeas : itECWS.measECWS)
				{
					...
					{
						TXYH2CCS::CCS2XYH(stationPos, fPointTransfo.getGeoid());
				...
				if (numberOfMeasurements > 0)
				{
					...
					itECVE->fPtLine = &fData.getPoints().addObject(
						LGCAdjustablePoint(TPositionVector(referencePoint[0], referencePoint[1], referencePoint[2], TCoordSysFactory::ECoordSys::k3DCartesian), false,
							false, true, "ECVE_line" + std::to_string(itECVE->line), fData.getConfig().referential, fData.getConfig().geoid.type, fTree.begin()));
				}
				else
