...
TLGCStatistic::TLGCStatistic() :
	fError(""), fWToCompute(true), fAreDetermined(nullptr), fGToCompute(nullptr), fDeltaComputed(nullptr), fZ(nullptr), fW(nullptr), fT(nullptr), fDelty(nullptr), fNablaValue(nullptr), fGValue(nullptr)
{
}
...
TLGCFautJsonData::TLGCFautJsonData(const TLGCData &project) : fProject(project)
{
}

void TLGCFautJsonData::serialize(ObjectSerializer &obj) const
{
	const auto &config = fProject.getConfig();
	const auto &stat = fProject.getStatistics();

	obj.addProperty("alpha", config.faut.alpha);
	obj.addProperty("beta", config.faut.beta);
	obj.addProperty("confidenceLevel", 1.0 - config.faut.alpha);
	obj.addProperty("testPower", 1.0 - config.faut.beta);
	obj.addProperty("sigmaZeroAposteriori", fProject.getS0APosteriori());
	obj.addProperty("sigmaZeroCriticalLow", fProject.getChiS0LowLimit());
	obj.addProperty("sigmaZeroCriticalHigh", fProject.getChiS0UpLimit());
	obj.addProperty("overallNetworkReliabilityFactor", stat.getOVERALL());
	obj.addProperty("networkDegreesOfFreedom", stat.getDOF());

	obj.addProperty("localReliability", stat.fZ);
	obj.addProperty("normalizedResiduals", stat.fW);
	obj.addProperty("internalReliability", stat.fT);
	obj.addProperty("externalReliability", stat.fDelty);
	obj.addProperty("smallestDetectableFault", stat.fNablaValue);
	obj.addProperty("grossErrorEstimate", stat.fGValue);
	obj.addProperty("observationDetermined", stat.fAreDetermined);
	obj.addProperty("grossErrorToCompute", stat.fGToCompute);
	obj.addProperty("externalReliabilityComputed", stat.fDeltaComputed);
	obj.addProperty("normalizedResidualsComputed", stat.getWToCompute());
}
