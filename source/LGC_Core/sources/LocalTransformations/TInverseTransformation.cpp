...
TInverseTransformation::TInverseTransformation() : TTransformation()
{
	setIdentityParams();
}
...
void TInverseTransformation::setTranslationPart(const TransformParameters &params)
{
	...
	m(0, 3) = -(kappacos * phicos) * params.tX - (kappacos * phisin * omegasin - kappasin * omegacos) * params.tY
		- (kappacos * phisin * omegacos + kappasin * omegasin) * params.tZ;
	m(1, 3) = -(kappasin * phicos) * params.tX - (kappasin * phisin * omegasin + kappacos * omegacos) * params.tY
		- (kappasin * phisin * omegacos - kappacos * omegasin) * params.tZ;
