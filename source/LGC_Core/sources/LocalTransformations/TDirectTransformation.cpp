...
TDirectTransformation::TDirectTransformation() : TTransformation()
{
	setIdentityParams();
}
...
TDenseMatrix TDirectTransformation::getPartialDerivativeWrtPosition() const
{
	...
	return (getMatrix().topLeftCorner(3, 3)) * getScaleFactor();
}
...
TDenseMatrix TDirectTransformation::getPartialDerivativeWrtHelmertParameters(const TPositionVector &pos) const
{
	...
	TDerivativeTransformation dtx = differentiatedTransformationTranslation(0);
	TDerivativeTransformation dty = differentiatedTransformationTranslation(1);
	TDerivativeTransformation dtz = differentiatedTransformationTranslation(2);
	TDerivativeTransformation drx = differentiatedTransformationAngle(0);
	TDerivativeTransformation dry = differentiatedTransformationAngle(1);
	TDerivativeTransformation drz = differentiatedTransformationAngle(2);
	...
	helmertDeriv.col(0) = toVector(dtx * pos);
	helmertDeriv.col(1) = toVector(dty * pos);
	helmertDeriv.col(2) = toVector(dtz * pos);
	helmertDeriv.col(3) = toVector(drx * pos);
	helmertDeriv.col(4) = toVector(dry * pos);
	helmertDeriv.col(5) = toVector(drz * pos);
