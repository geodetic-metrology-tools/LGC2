...
bool TReader::expandDeformDirectives()
{
	...
	// Helper lambda: expand a single point (rename to _ref, create free POIN, create sag pair)
	auto expandPoint = [&](const std::string &originalName, const std::string &sagElementName, int lineNum) -> bool {
		...
		{
			outputMessages << TFileLogger::e_logType::LOG_ERROR
						   << "Line " + std::to_string(lineNum) + ": DEFORM cannot rename point '" + originalName + "' to '" + refName + "' because that name already exists.";
			return false;
		}
		...
		// Get properties before renaming
		...
		// Rename original point: "A" -> "A_ref" (preserves all original properties)
		if (!project.getPoints().renameObject(originalName, refName))
		{
			outputMessages << TFileLogger::e_logType::LOG_ERROR
						   << "Line " + std::to_string(lineNum) + ": DEFORM failed to rename point '" + originalName + "'.";
			return false;
		}

		// Mark the renamed reference point as auto-generated so writers can skip it
		project.getPoints().getObject(refName).setAutoGenerated(true);

		// Create new free POIN with the original name, same coordinates, same frame
		LGCAdjustablePoint newPoint(provValue, false, false, false, originalName, refFrame, framePos);
		project.getPoints().addObject(newPoint);
