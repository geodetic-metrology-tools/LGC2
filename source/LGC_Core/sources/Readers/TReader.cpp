...
bool TReader::read(std::istream &lgcStream)
{
	...
	if (!expandDeformDirectives())
		outputMessages << TFileLogger::e_logType::LOG_ERROR << "DEFORM directive expansion failed.";
...
bool TReader::expandDeformDirectives()
{
	...
	// Helper lambda: expand a single point into a _ref_<sagname> + sagged pair.
	// The original point object is kept in place (preserving any observation pointers)
	// and converted to a free sagged point. A new _ref_<sagname> point is created with the original's fixed state.
	// The sag element name is appended to the suffix so the generated reference point cannot collide
	// with user-defined points that happen to use a "_ref" suffix for other purposes.
	auto expandPoint = [&](const std::string &originalName, const std::string &sagElementName, int lineNum) -> bool {
		std::string refName = originalName + "_ref_" + sagElementName;
		...
		project.getConfig().updatePointNameWidth(refName);
	...
	// Each point may appear at most once per role (reference / associated).
	{
		...
		if (!checkUniqueMembership("associated", [](const TLGCSagConstraintPair &p) { return p.getAssocPoint(); }))
			return false;
		if (!checkUniqueMembership("reference", [](const TLGCSagConstraintPair &p) { return p.getRefPoint(); }))
			return false;
	}
