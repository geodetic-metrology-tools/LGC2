...
bool TLSResultsMatricesExtractor::extractResiduals(const TLSResultsMatrices &rm)
{
	...
	try
	{
		...
		for (TDataTreeIterator itTree = fDataSet->getTree().begin(); itTree != fDataSet->getTree().end(); itTree++)
		{
			...
			// In every node iterate through the ROLLYROM's measurements
			for (auto &itROLLY : itTree.node->data->measurements.fROLLY)
				extractROLLYROMObs(rm, itROLLY);
 ...
 *       data structure for subsequent analysis, reliability assessment, and output
 *       generation. The function includes comprehensive error checking to detect
 *       indexing mismatches that could indicate serious calculation problems.
 */
void TLSResultsMatricesExtractor::extractROLLYROMObs(const TLSResultsMatrices &rm, TROLLYROM &rollyMeas)
{
	// Process each ROLLY measurement in the round to extract computed residuals
	for (auto &itROLLY : rollyMeas.measROLLY)
	{
		...
		if (obsUidx < rm.getResidualsVectByConst()->size())
		{
			// Extract the computed residual value from the results matrices at the specified index
			// and update the measurement object with the calculated residual
			// The residual represents the difference between observed and estimated values
			itROLLY.setAngleResidual(TAngle(rm.getResidualsVctrElmt(obsUidx)));
		}
		else
		{
...
void TLSResultsMatricesExtractor::extractSagVarCovar(const TLSResultsMatrices &rm)
{
	for (auto &sagObj : fDataSet->getSags())
	{
		...
		if (!vertCurv.isFixed())
		{
			int unknIdx = vertCurv.getFirstUidx();
			vertCurv.setEstimatedPrecision(vertCurv.getFirstUidx(), sqrtq(rm.getUnkCovarMtrxElmt(unknIdx, unknIdx)));
		}
		auto &vertSag = sagObj.getVertSag();
		if (!vertSag.isFixed())
		{
			int unknIdx = vertSag.getFirstUidx();
			vertSag.setEstimatedPrecision(vertSag.getFirstUidx(), sqrtq(rm.getUnkCovarMtrxElmt(unknIdx, unknIdx)));
		}
		auto &radCurv = sagObj.getRadCurv();
		if (!radCurv.isFixed())
		{
			int unknIdx = radCurv.getFirstUidx();
			radCurv.setEstimatedPrecision(radCurv.getFirstUidx(), sqrtq(rm.getUnkCovarMtrxElmt(unknIdx, unknIdx)));
		}
		auto &radSag = sagObj.getRadSag();
		if (!radSag.isFixed())
		{
			int unknIdx = radSag.getFirstUidx();
			radSag.setEstimatedPrecision(radSag.getFirstUidx(), sqrtq(rm.getUnkCovarMtrxElmt(unknIdx, unknIdx)));
		}
