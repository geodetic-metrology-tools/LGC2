				...
				for (auto itROM : itTSTN->roms) {
					for (auto& itANGL : itROM->measANGL)
					{
						extractAngleObs(rm, itANGL); //Extract residuals of ANGL
													 ...
													 // Calculates and sets the equivalent offset (offset estimated at the target level)
						TLength dist = itTSTN->instrumentPos->getEstimatedEuclideanDistance(itANGL.targetPos);
						itANGL.setAngleResidualOffset(TLength(dist.getMetresValue() * itANGL.getAngleResidual().getRadiansValue()));
					}
					...
					for (auto& itZEND : itROM->measZEND)
					{
						extractAngleObs(rm, itZEND); //Extract residuals of ZEND, same extractor used for both angle measurements
													 ...
													 // Calculates and sets the equivalent offset (offset estimated at the target level)
						TLength dist = itTSTN->instrumentPos->getEstimatedEuclideanDistance(itZEND.targetPos);
						itZEND.setAngleResidualOffset(TLength(dist.getMetresValue() * itZEND.getAngleResidual().getRadiansValue()));
					...
					for (auto& itPLR3D : itROM->measPLR3D)
					{
						extractPLR3DObs(rm, itPLR3D);  //Extract residuals of PLR3D measurement
													   ...
													   // Calculates and sets the equivalent offset(offset estimated at the target level)
						TLength dist = itTSTN->instrumentPos->getEstimatedEuclideanDistance(itPLR3D.targetPos);
						TLength eAngl = TLength(dist.getMetresValue() * itPLR3D.getAngleResidual(EPLR3DAngles::kANGL).getRadiansValue());
						TLength eZend = TLength(dist.getMetresValue() * itPLR3D.getAngleResidual(EPLR3DAngles::kZEND).getRadiansValue());
						itPLR3D.setAngleResidualOffset(eAngl, EPLR3DAngles::kANGL);
						itPLR3D.setAngleResidualOffset(eZend, EPLR3DAngles::kZEND);
					}
