...
//	dxTrial.setConstant(1);
	...
	while (dx.norm() > 1e-6)
	{
		...
		while (!acceptLMStep)
		{
			...
			std::cout << "proposed |dx_inner|=" << dxTrial.norm() << std::endl;
...
TSparseMatrix TLSGaussNewton::getNBig(TSparseMatrix &N2, TSparseMatrix &C, double lmCoefficient)
{
	...
	coeffs.reserve(N2.nonZeros() + 2 * C.nonZeros());
	...
	for (int k = 0; k < C.outerSize(); ++k)
	{
		...
		{
			coeffs.push_back(TTriplet(it.row() + N2.rows(), it.col(), it.value())); // C
			coeffs.push_back(TTriplet(it.col(), it.row() + N2.cols(), it.value())); // CT
		}
	}
