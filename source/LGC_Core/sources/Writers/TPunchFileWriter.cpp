...
// Column header label for frame names
static const std::string kFrameNameLabel = "FRAME_NAME";
...
void TPunchFileWriter::writeFrameSectionHeader()
{
	TAStreamFormatter *stream = getStream();
	int nameWidth = getNameWidth();
	int frameNameWidth = std::max(nameWidth, (int)kFrameNameLabel.length() + 1);
	int paramWidth = getObsWidth() + 8;
	int coefWidth = 6;
	/////////////////////////////////////////////////////////////////////////////////////////////
	// Header line 1
	(*stream) << "%";
	(*stream).writeStringLeft(frameNameWidth, kFrameNameLabel);
	(*stream).writeStringLeft(frameNameWidth, "FRAME_ID");
	...
	// Header line 2
	(*stream) << "%";
	(*stream).writeStringLeft(frameNameWidth, "");
	(*stream).writeStringLeft(frameNameWidth, "");
...
void TPunchFileWriter::writeFrameSectionData()
{
	...
	for (TDataTreeIterator itTree = fProjectData->getTree().begin(); itTree != fProjectData->getTree().end(); itTree++)
	{
		if (itTree->get()->frame.getName() != "ROOT")
		{
			...
			(*stream).width(1);
			(*stream) << "";
			(*stream).writeStringLeft(frameNameWidth, itTree->get()->frame.getName());
			(*stream).writeStringLeft(frameNameWidth, nameID);
