...
void TFRAMEWriter::writeFRAMEDefinition(const TAdjustableHelmertTransformation &frame)
{
	...
	if (!frame.isFixed())
	{
		...
		{
			(*stream) << TABs;
			(*stream).writeStringLeft(nameWidth, "XY ROT. COVAR (GON^2): ");
		...
		{
			(*stream) << TABs;
			(*stream).writeStringLeft(nameWidth, "YZ ROT. COVAR (GON^2): ");
		...
		{
			(*stream) << TABs;
			(*stream).writeStringLeft(nameWidth, "XZ ROT. COVAR (GON^2): ");
		...
		{
			(*stream) << TABs;
			(*stream).writeStringLeft(nameWidth, "XY TRANS. COVAR (m^2): ");
		...
		{
			(*stream) << TABs;
			(*stream).writeStringLeft(nameWidth, "YZ TRANS. COVAR (m^2): ");
		...
		{
			(*stream) << TABs;
			(*stream).writeStringLeft(nameWidth, "XZ TRANS. COVAR (m^2): ");
...
void TFRAMEWriter::writeINCLReliabilityHelper(const MeasurementContainer &measurements, const char *sectionTitle, HeaderFunc writeHeaderFunc, DataFunc writeDataFunc)
{
	...
	// Single unified for loop - handles both INCLY and ROLLY!
	for (const auto &rom : measurements)
	{
		...
		if (measList.size() > 0)
		{
			...
			{
				(*stream) << "\n" << sectionTitle << "\n";
				(inclWriter.*writeHeaderFunc)();
				writeHeader = false;
			}
			// Write reliability data for each round of measurements
			(inclWriter.*writeDataFunc)(rom, fProjectData->getStatistics(), measList);
		}
	}
}
...
{ // FILE.OUTPUT
	...
	{ // It is ROOT
		...
		if (globalRef != TRefSystemFactory::ERefFrame::kLocalRefFrame && fProjectData->getConfig().geoid.type != TRefSystemFactory::EGeoid::kCUSTOMgeoid
			&& fProjectData->getConfig().geoid.type != TRefSystemFactory::EGeoid::kCGSphere)
		{
