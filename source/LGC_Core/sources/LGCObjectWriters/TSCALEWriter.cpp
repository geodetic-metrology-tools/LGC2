...
void TSCALEWriter::writeECHOResults(const TECHOROM &echorom)
{
	...
	(*stream).writeDouble(obsWidth, lengthPrecision,
		echorom.fMeasuredPlane->getReferencePoint()->getEstValue(0)
			+ echorom.fMeasuredPlane->getRefPtDistEstimatedValue().getMetresValue() * sin(echorom.fMeasuredPlane->getThetaEstimatedValue().getRadiansValue() + M_PI_2));
	(*stream).writeStringLeft(nameWidth, "Y (M)");
	(*stream).writeDouble(obsWidth, lengthPrecision,
		echorom.fMeasuredPlane->getReferencePoint()->getEstValue(1)
			+ echorom.fMeasuredPlane->getRefPtDistEstimatedValue().getMetresValue() * cos(echorom.fMeasuredPlane->getThetaEstimatedValue().getRadiansValue() + M_PI_2));
	// Z is not relevant
	//(*stream).writeStringLeft(nameWidth, "Z (M)");
	//(*stream).writeDouble(obsWidth, lengthPrecision, echorom.fMeasuredPlane->getReferencePoint()->getEstValue(2));
	(*stream) << endl;
...
void TSCALEWriter::writeECVEResults(const TECVEROM &ecverom)
{
	...
	for (auto const &ItECVE : ecverom.measECVE)
	{
		...
		// write the residual/sigma
		(*stream).writeDouble(obsResWidth, lengthResPrecision,
			ItECVE.getDistanceResidual().getMetresValue() / ItECVE.target.sigmaCombinedDist.getMetresValue()); // Output value in meters [m], stored in [m]
