...
TLGCObsSummary TLGCObsSummary::merge(const std::list<const TLGCObsSummary *> &summaries)
{
	...
	for (const auto &sum : summaries)
	{
		...
		// Check if residual is smaller or bigger than current resMin or resMax:
		if (sum->fResMax > summary.fResMax)
			summary.fResMax = sum->fResMax;
		if (sum->fResMin < summary.fResMin)
			summary.fResMin = sum->fResMin;
...
#if USE_SERIALIZER
void TLGCObsSummary::serialize(ObjectSerializer &obj) const
{
