...
{
	//Separate 
	for (AdjPointIter it(fProjectData->points.objects.begin()); it != fProjectData->points.objects.end(); ++it){	
...
void TFRAMEConverter::writePointType(const std::list<AdjPointIter>& lop, TDataTreeIterator frameIt, TSpatialStatus::ESpatialStatus type, bool localNode){
	if(!lop.empty()){
		...
		if(!localNode)
			writeResultsPtsHeader(type, lop.size(), fProjectData->cfg.referential.typeName(), localNode);
...
void	TFRAMEConverter::writeResultsPtsHeader(const TSpatialStatus::ESpatialStatus status, const int ptNumber, const string &refSys, bool isLocal)
{
	...
	TPointConverter converter (stream, fProjectData->cfg.referential.type()); 
...
{//FILE.OUTPUT
	...
	if(isLocal){
		...
		TDataTreeIterator root = fProjectData->tree.begin();
		TLGCRefFrame::ERefs globalRef = fProjectData->cfg.referential.type();
		...
		if(root != pt->getFrameTreePosition()){
			TLOR2LOR transfo = TLOR2LOR(pt->getFrameTreePosition(), fProjectData->tree.begin(), "transfo");
