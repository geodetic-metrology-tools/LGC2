												  ...
												  string name3)
{
	...
	// get Point 1
	(*stream).writeStringLeft(nameWidth, name1);

	//get Point 2
	(*stream).writeStringLeft(nameWidth, name2);

	// get Point 3
	(*stream).writeStringLeft(nameWidth, name3);
...
void	TLengthObsConverter::readDistStation(TDistMeasStation& distStation)
{
	TAStreamFormatter*	stream = getStream();
	...
	TDistMeasStation* station = new TDistMeasStation(stName);
	distStation = *station;
