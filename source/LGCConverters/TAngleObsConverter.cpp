...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
void	TAngleObsConverter::writeReliabilityData(const TAMeas<TInstrumentData::TPOLAR::TTarget,ENoValues, 0,ESingleValue, 1>& fAngle)
{
	...
	//obsIt.getAngle()+obsIt.getAngleResidual()).gon

	// get Point 1
	//(*stream).writeStringLeft(nameWidth, absAng.getStPointName());

	//get Point 2
	(*stream).writeStringLeft(nameWidth, fAngle.targetPos->getName());
	...
	//(*stream) << fAngle.getAngle().gon << separator;
...
{//inputs a horizontal angle measurement from the text stream
	...
	if(!stream->hasNoObsToRead() && !isAKeyChar)
	{
		TAngle::EUnits oldunit = stream->TAStreamFormatter::setAngleUnits(TAngle::kCCs);
		stream->TAStreamFormatter::operator>>(sigma);
		stream->TAStreamFormatter::setAngleUnits(oldunit);
...
#if 0
bool	TAngleObsConverter::readColSigma(TReal &colSigma, int& i, TLGCProject*)
{
	TAStreamFormatter*	stream = getStream();
	...
	stream->skipWhiteSpace();
	if (!stream->hasNoObsToRead() && !stream->atEnd() && stream->peek() != '\n')
	{
		*stream >> colSigma;
...
TAStreamFormatter& TAngleObsConverter::readTheoStation( TTheodoliteStation& thStation)
{//inputs a theodolite station from the text stream
	TAStreamFormatter*	stream = getStream();

	TSpatialPointName stName;
	stream->TAStreamFormatter::operator>>(stName);

	TTheodoliteStation* station = new TTheodoliteStation(stName);
	thStation = *station;
	delete station;
...
void	TAngleObsConverter::setConstant(TAngleConstants cte, TTheodoliteStation* newThSt, TLGCProject* project)
{
	// if necessary, add the constant to the Working Constants
	if (project->getDataSet()->getWorkingConstants()->numberOfAngleConstants() == 0)
	{
		project->getDataSet()->getWorkingConstants()->addAngleConstant(&cte);
	}
	else
	{
		AngleConstantIterator constIter = --(project->getDataSet()->getWorkingConstants()->getAngleConstantsEndIterator());
	...
	newThSt->ifNotDoneSetAngleConst(cte);
	return;
}
