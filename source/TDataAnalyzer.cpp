...
bool TDataAnalyzer::dataConsistent(){
	...
	int nCALAinROOT = 0;
	auto& outputMessages(fData.fOutputFileWriter);
	...
	auto pdor(fData.cfg.pdor);
	if(pdor.isActive()){
		if(fData.points.doesObjectExist(pdor.fptname)){
			auto& pdorPoint(fData.points.getObject(pdor.fptname));
	...
	for (auto it( fData.tree.begin()); it != fData.tree.end(); ++it){	
		...
		for (auto itLEVEL( it.node->data.get()->measurements.fLEVEL.begin()); itLEVEL != it.node->data.get()->measurements.fLEVEL.end(); ++itLEVEL){
			if(!itLEVEL->fMeasuredPlane->isInitialized()){
				...
				for (auto itDLEVMeas( itLEVEL->measDLEV.begin()); itDLEVMeas != itLEVEL->measDLEV.end(); ++itDLEVMeas){
					referencePoint[0] += itDLEVMeas->targetPos->getEstimatedValue().getX().getValue();
					referencePoint[1] += itDLEVMeas->targetPos->getEstimatedValue().getY().getValue();
					referencePoint[2] += itDLEVMeas->targetPos->getEstimatedValue().getZ().getValue();
				}
				...
				if(numberOfMeasurements>0){
					...
					TAdjustablePoint& rp =
						fData.points.addObject(TAdjustablePoint(TPositionVector(referencePoint[0], referencePoint[1], referencePoint[2],TCoordSysFactory::ECoordSys::k3DCartesian), 
						false, false, true, "RefPtDLEV" + levelCounter, TLGCRefFrame::ERefs::kOLOC, fData.tree.begin()));
	...
	for (std::list<TAdjustablePoint>::iterator it = fData.points.objects.begin(); it != fData.points.objects.end(); ++it){	
	...
	for (auto it = fData.scalars.objects.begin(); it != fData.scalars.objects.end(); ++it){		
		if(!it->isInitialized()){
			consistent = false;
	...
	for (auto it = fData.angles.objects.begin(); it != fData.angles.objects.end(); ++it){		
		if(!it->isInitialized()){
			consistent = false;
	...
	for (auto it = fData.lines.objects.begin(); it != fData.lines.objects.end(); ++it){		
		if(!it->isInitialized()){
			consistent = false;
	...
	for (auto it = fData.planes.objects.begin(); it != fData.planes.objects.end(); ++it){		
		if(!it->isInitialized()){
			consistent = false;
...
TCalcType TDataAnalyzer::getCalculationType(){
	...
	if(configuration.allfixed.isActive()){
		for (auto it = fData.points.objects.begin(); it != fData.points.objects.end(); ++it){		
	...
	cType.fCombinedCase = fData.combinedCaseUsed();  //Tells if combined case LS model needed (if PLR3D or DIR3D measurements used)
